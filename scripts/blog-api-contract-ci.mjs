...
const baseUrl = (process.env.BACKY_PUBLIC_BASE_URL || 'http://localhost:3001').replace(/\/$/, '');
...
const waitForDiscovery = async (serverState) => {
  const deadline = Date.now() + Number(process.env.BACKY_BLOG_API_CI_SERVER_TIMEOUT_MS || 60000);
  let lastError = null;
...
try {
  ...
  await runStep('Blog/API contract smoke', ['run', 'test:smoke:blog-api'], {
    env: {
      BACKY_PUBLIC_BASE_URL: baseUrl,
    },
  });
} finally {
