...
- Media upload now has runtime admin/public APIs plus a provider-neutral storage adapter contract, but production still needs route-level adapter wiring, quotas, transforms, signed/private delivery, and per-site ownership enforcement.
...
| Media library | Durable uploads, metadata, attachment usage | Runtime media endpoints and admin `/media` now cover upload/list/update/delete, folders, custom upload metadata, visibility, usage links, and uploaded font registration metadata; `@backy/storage` defines local/S3/R2/Supabase upload/read/delete/sign/list/stat adapters with local smoke coverage; public render payloads expose public font assets and hosted pages inject `@font-face` rules | Wire API uploads to the storage adapter, add quotas, transforms, signed/private delivery, font variant grouping, and stronger bulk/reference management |
