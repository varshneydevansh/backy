...
| Revisions | Content and template revisions | Page/post revision endpoints, editor rollback panels, page/blog list summary cells, visible editor actor/action metadata, and handoff revision metadata now expose saved snapshot counts/latest notes with history deep links; PageVersion type/schema still lacks richer graph/rendered visual depth | Add branching graph depth, pixel-level visual comparisons, publish snapshots, and audit metadata |
