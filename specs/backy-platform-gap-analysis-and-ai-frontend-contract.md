...
| Presets/templates | Starter CMS presets create sample collection plus list/detail pages | Editor component library now supports composed nested presets and includes hero, feature-grid, and lead-capture form blocks that save as normal canvas child trees. A complete persisted site/page/blog/form template registry is still absent. | Add site/page/blog/form template registry, cloning/versioning, saved user sections, and dynamic list/detail page presets |
...
| Patterns/reusable blocks | Saved patterns can be inserted, synced, managed, exported | Static composed presets now exist for hero, feature-grid, and lead-capture form blocks, but there is no saved/synced pattern registry yet | Add reusable blocks, synced sections, block/template registry, JSON export/import |
