...
- Media upload now has runtime admin/public APIs, environment-selected local/S3/Supabase-compatible storage, quotas, signed private delivery, generated responsive WebP transform files, DB-backed retained-version create/list/compare/restore/delete paths with checksum/path/name/timeline diff review, static upload safety checks, optional HTTP scanner and ClamAV `clamd` scanning, quarantine controls, manual provider/CDN metric intake with conversions/value/attribution windows, provider analytics batch ingestion by media id/storage path/URL, and admin audit logging. Production still needs provider-side provisioning/credential rotation, visual/audio-aware binary diff previews, deeper funnel/ROI attribution dashboards, and per-site ownership enforcement.
