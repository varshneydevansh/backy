...
| Presets/templates | Starter CMS presets create sample collection plus list/detail pages | Editor component library now supports composed nested presets and includes hero, feature-grid, and lead-capture form blocks that save as normal canvas child trees. Admin reusable-section CRUD APIs now persist saved canvas sections with category/tag/status filters in the runtime adapter. A complete editor save/insert flow and persisted site/page/blog/form template registry are still absent. | Add editor UI for saving/inserting reusable sections, site/page/blog/form template registry, cloning/versioning, synced sections, and dynamic list/detail page presets |
...
| Patterns/reusable blocks | Saved patterns can be inserted, synced, managed, exported | Static composed presets now exist for hero, feature-grid, and lead-capture form blocks, and admin reusable-section CRUD APIs now store saved canvas section patterns. The editor still cannot save/insert those API-backed patterns, and synced reusable instances are not implemented yet. | Add editor reusable-section UI, synced sections, block/template registry, JSON export/import |
