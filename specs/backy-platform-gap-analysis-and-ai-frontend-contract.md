...
|---|---|---|---|
| Collections | Structured content collections independent of pages | Generic collection schema/record admin APIs, public read APIs, and a backend-backed admin `/collections` builder now exist in the runtime adapter. The builder supports schema fields, select/tag option metadata, reference target metadata, public-read status, record CRUD, record search/filter/sort controls, backend-paginated record lists, selected-record bulk status/delete actions, filtered CSV import/export, and dynamic route links; required/unique field validation, select/tag option validation, public visibility guards, CSV export, CSV import, admin record pagination metadata, and bulk record actions are covered by `test:admin-contract`. Durable DB querying is still absent. | Add DB-backed indexes/querying, backups, audit events, and full RBAC permissions |
...
| Presets/templates | Starter CMS presets create sample collection plus list/detail pages | Editor component library now supports composed nested presets and includes hero, feature-grid, and lead-capture form blocks that save as normal canvas child trees. Admin reusable-section CRUD APIs persist saved canvas sections with category/tag/status filters, and the editor library can save the selected element tree plus reload/insert/rename/delete active saved sections. A complete persisted site/page/blog/form template registry is still absent. | Add site/page/blog/form template registry, cloning/versioning, synced sections, richer saved-section metadata editing, and dynamic list/detail page presets |
...
| Patterns/reusable blocks | Saved patterns can be inserted, synced, managed, exported | Static composed presets now exist for hero, feature-grid, and lead-capture form blocks. Admin reusable-section CRUD APIs store saved canvas section patterns, and the editor can save/insert/rename/delete active site sections as cloned editable canvas trees. Synced reusable instances are not implemented yet. | Add synced sections, richer saved-section metadata editing, block/template registry, JSON export/import |
