...
| Dataset binding | Page elements bind to collection fields | Collection-bound element `dataBindings` now normalize into `/render` dataset/binding manifests with resolved public field schemas and records for record/slug-bound datasets; admin binding UI is still absent | Add admin dataset binding UI, richer query/filter/sort rules, and DB-backed dataset resolution |
