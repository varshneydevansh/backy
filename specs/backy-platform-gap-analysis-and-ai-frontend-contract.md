...
- Media upload now has runtime admin/public APIs, environment-selected local/S3/Supabase-compatible storage, quotas, signed private delivery, generated responsive WebP transform files, DB-backed retained-version create/list/compare/restore/delete paths, static upload safety checks, optional HTTP scanner and ClamAV `clamd` scanning, quarantine controls, and admin audit logging. Production still needs provider-side provisioning/credential rotation, richer binary diff workflows, provider/CDN analytics, and per-site ownership enforcement.
