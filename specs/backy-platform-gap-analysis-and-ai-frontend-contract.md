...
| Dataset binding | Page elements bind to collection fields | Collection-bound element `dataBindings` now normalize into `/render` dataset/binding manifests with resolved public field schemas and records for record/slug-bound datasets. The editor property panel now provides a basic collection/field/target binding UI for selected elements. | Add richer dataset query/filter/sort/repeater rules and DB-backed dataset resolution |
