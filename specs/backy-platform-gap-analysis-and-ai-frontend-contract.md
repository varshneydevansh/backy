...
- Media upload now has runtime admin/public APIs, environment-selected local/S3/Supabase-compatible storage, quotas, signed private delivery, generated responsive WebP transform files, DB-backed retained-version create/list/restore/delete paths, static upload safety checks, optional HTTP scanner and ClamAV `clamd` scanning, quarantine controls, and admin audit logging. Production still needs provider-side provisioning/credential rotation, richer binary diff/compare workflows, provider/CDN analytics, and per-site ownership enforcement.
...
- Media: backend and UI now cover runtime storage, metadata, folders, replacement, retained-version listing/restore/deletion, bindings, safety-scan state, optional HTTP and ClamAV scanning, quarantine controls, generated responsive manifests, quotas, signed/private delivery, audit activity, usage analytics, and Backy-served delivery counts; still needs provider-side provisioning/credential rotation, richer binary diff/compare workflows, and provider/CDN analytics.
