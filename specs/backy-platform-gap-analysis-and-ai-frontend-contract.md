...
| Dynamic pages | One template renders collection list and many item URLs | Page/blog routes exist, public `/:collectionSlug` plus `/:collectionSlug/:recordSlug` routes now resolve/render generated collection list/item payloads after normal pages/blog posts, and admin page/collection writes reject conflicting route patterns. Backy-authored list/item page canvases can now be captured into collection metadata and used by dynamic collection routes when no captured frontend-design template is attached, and the Collections UI resolves preview record URLs for hosted and render API list/item routes. The Collections UI now also persists `backy.collection-slug-policy.v1` metadata with source/fallback/update controls, computes `backy.collection-slug-policy-readiness.v1` against `:recordSlug` routes and preview records, and copies a `backy.collection-slug-policy-action-plan.v1` for custom frontend create/import forms. | Add deeper admin controls for list/detail page design and broader visual coverage |
...
| Revisions | Content and template revisions | Page/post revision endpoints, editor rollback panels, page/blog list summary cells, visible editor actor/action metadata, enriched page/blog `backy.*-revision-graph.v1` node metadata, and handoff revision metadata now expose saved snapshot counts/latest notes with history deep links; PageVersion type/schema still lacks true branching graph/rendered visual depth | Add true branching graph depth, pixel-level visual comparisons, publish snapshots, and audit metadata |
...
- Editor: responsive overrides, layer controls, multi-select operations, composed presets, editor-level synced reusable-section instances, reusable-section instance propagation, conflict-safe page saves, grouping, keyboard shortcuts, long-session recovery, and inspector-level `backy.editor-composition-readiness.v1` / `backy.editor-composition-action-plan.v1` handoff metadata are implemented and covered by focused smokes. Page editor saves send `expectedUpdatedAt`, stale saves return `PAGE_VERSION_CONFLICT`, and the editor renders a reload-latest recovery action. Remaining editor risk is continued canonical schema convergence and regression coverage as new block controls ship.
