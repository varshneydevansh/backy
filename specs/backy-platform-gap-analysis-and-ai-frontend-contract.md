...
| Patterns/reusable blocks | Saved patterns can be inserted, synced, managed, exported | Static composed presets now exist for hero, feature-grid, and lead-capture form blocks. Admin reusable-section CRUD APIs store saved canvas section patterns, and the editor can save/insert active site sections as cloned editable canvas trees. Synced reusable instances are not implemented yet. | Add synced sections, saved-section management UI, block/template registry, JSON export/import |
