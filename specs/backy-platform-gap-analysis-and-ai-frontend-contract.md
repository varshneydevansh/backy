...
| Media library | Durable uploads, metadata, attachment usage | Runtime media endpoints and admin `/media` now cover upload/list/update/delete, folders, custom upload metadata, visibility, usage links, and uploaded font registration metadata; public render payloads expose public font assets and hosted pages inject `@font-face` rules | Add storage adapter, quotas, transforms, signed/private delivery, font variant grouping, and stronger bulk/reference management |
