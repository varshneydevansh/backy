...
- Media upload now has runtime admin/public APIs and route-level local storage adapter wiring, but production still needs remote provider configuration, quotas, transforms, signed/private delivery, and per-site ownership enforcement.
...
| Patterns/reusable blocks | Saved patterns can be inserted, synced, managed, exported | Static composed presets now exist for hero, feature-grid, and lead-capture form blocks. Admin reusable-section CRUD APIs store saved canvas section patterns; the editor can save/insert/rename/delete active site sections as cloned editable canvas trees; and active saved sections are exposed to custom frontends through public list/detail APIs, manifest/OpenAPI, and the SDK. Synced reusable instances are not implemented yet. | Add synced sections, richer saved-section metadata editing, block/template registry, JSON export/import |
| Media library | Durable uploads, metadata, attachment usage | Runtime media endpoints and admin `/media` now cover upload/list/update/delete, folders, custom upload metadata, visibility, usage links, and uploaded font registration metadata; admin upload/delete routes write through `@backy/storage` local adapter while preserving `/uploads/sites/:siteId/...` public URLs; `@backy/storage` defines local/S3/R2/Supabase upload/read/delete/sign/list/stat adapters with local smoke coverage; public render payloads expose public font assets and hosted pages inject `@font-face` rules | Add remote storage provider selection, quotas, transforms, signed/private delivery, font variant grouping, and stronger bulk/reference management |
...
| REST/headless | Stable JSON APIs with auth rules | Public endpoints exist, `/api/sites/:siteId/manifest` now provides a site-level frontend discovery document with schema refs, capabilities, route patterns, endpoint URLs, module summaries, collections, forms, media/font counts, and navigation, and `/api/sites/:siteId/openapi` exports a site-scoped OpenAPI 3.1 document for the public frontend surface. Runtime data still uses the seeded/JSON adapter and some legacy endpoints have inconsistent envelopes. | Add SDK examples, CORS, API keys, read/write route separation, and DB-backed services |
