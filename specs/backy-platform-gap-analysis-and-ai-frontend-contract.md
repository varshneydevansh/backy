...
Backy is not yet a complete Wix/Canva/WordPress-class CMS platform, but the current page-surface audit has moved most primary admin/editor/API surfaces out of prototype status. The biggest remaining risk is no longer generic UI scaffolding; it is proving the remaining live-provider gates without confusing demo/mock-provider coverage for production certification.
...
- Public data can still run in seeded/demo mode, but the remaining production risk is live provider certification; the configured Supabase/Postgres service-data gates for Forms and SDK manifests passed against a migrated disposable Postgres target on 2026-05-21.
...
- Media upload now has runtime admin/public APIs, environment-selected local/S3/Supabase-compatible storage, quotas, signed private delivery, generated responsive WebP transform files, DB-backed retained-version create/list/compare/restore/delete paths with checksum/path/name/timeline diff review plus media-aware previews, static upload safety checks, optional HTTP scanner and ClamAV `clamd` scanning, quarantine controls, manual provider/CDN metric intake with conversions/value/attribution windows, media.configure-gated provider metadata controls, media.edit-gated mutation routes, site/team-scoped ownership enforcement through central admin access checks, provider analytics batch ingestion by media id/storage path/URL, library-wide provider ROI rollups, a copyable `backy.media-attribution-handoff.v1` contract for cross-channel media reporting, and admin audit logging. Production still needs automated provider-account bucket creation/secret rotation; the current media flow validates the active runtime bucket/path and gives operators a credential-rotation runbook without storing secrets.
...
| Dataset binding | Page elements bind to collection fields | Collection-bound element `dataBindings` now normalize into `/render` dataset/binding manifests with resolved public field schemas and records for record/slug/search/filter/sort-bound datasets, and Backy's public payload/hosted renderer now resolves bound values into element props, including multi-hop reference joins such as `author.company.name`. Render payloads also support a collection-backed `repeater` element contract that hydrates records into `props.records`, materializes joined repeater field paths such as `record.values["author.company.name"]`, resolves dynamic item reverse-relationship filters through `$currentRecord.id`, and exposes the same dataset manifest for custom frontends. The editor property panel provides collection/field/target binding controls, reusable title/summary/image/link binding presets, site-scoped shared saved preset management backed by `/api/admin/sites/:siteId/editor/collection-binding-presets` with local fallback, nested reference-field join controls, selected-record value previews with image thumbnails plus resolved joined values, recent-record selection, manual record id/slug override, and query/filter/sort/pagination controls for selected elements, plus first-class repeater dataset, join-aware field mapping, joined query filter/sort controls, current-record reference filter controls, query-backed matching-record previews with joined title/summary/image resolution, and grid layout controls. Preset binding slots can now be bulk-applied from the Data panel across root slots, named descendants, repeaters, and child element slots in one action for the selected collection, collection bindings can be cleared across the selected preset tree when editors need to remap a composed section, slot coverage now summarizes matched, required, applied, and existing bound targets before publish or custom frontend handoff, editors can copy a `backy.editor.binding-slot-coverage.v1` JSON brief for AI/custom frontend builders, selected elements can copy a `backy.editor.dataset-binding.v1` brief with collection, field, target, join, preview-record, public records URL, query settings, and `backy.editor.dataset-binding-action-plan.v1`, and repeaters can copy a `backy.editor.repeater-dataset.v1` brief with dynamic list mapping, public records URL, query, layout, preview rows, and `backy.editor.repeater-dataset-action-plan.v1`. The `/collections` surface now includes a visual relationship browser for outgoing references, incoming reverse references, unmapped relationship warnings, linked schema counts, route hints, controls to attach a captured frontend collection template to an existing schema, and dataset authoring shortcuts that copy repeater presets, field-binding presets, list-page briefs, item-page briefs, and list/item page-builder handoff routes. `/pages/new` now consumes `collectionId` plus `datasetMode`, renders a dataset import panel, includes collection field mappings in the creation handoff manifest, exposes copyable `backy.page-create-dataset-readiness.v1` / `backy.page-create-dataset-action-plan.v1` briefs for selected collection, mode, route, field roles, generated canvas expectations, public records/manifest/OpenAPI endpoints, and permissions, and seeds collection-backed list pages with repeater props or item pages with bound title/summary elements. The `/collections` surface can also capture Backy-authored page canvases as collection list/item templates and publish them through dynamic routes when no higher-priority captured frontend template is attached, exposes preview-record controls that resolve hosted/render list and item URLs from real records, keeps a bounded capture history for authored list/item templates with restore controls, and compares active captures against saved history versions with root-element, page, canvas-size, custom-CSS/JS, and design-state diff summaries. | Add deeper in-editor dataset UX |
| Dynamic pages | One template renders collection list and many item URLs | Page/blog routes exist, public `/:collectionSlug` plus `/:collectionSlug/:recordSlug` routes now resolve/render generated collection list/item payloads after normal pages/blog posts, and admin page/collection writes reject conflicting route patterns. Backy-authored list/item page canvases can now be captured into collection metadata and used by dynamic collection routes when no captured frontend-design template is attached. Those authored captures preserve custom CSS/JS, content documents, theme-token refs, assets, animation arrays, interactions, data bindings, editable maps, SEO, metadata, and root elements, and public dynamic render payloads expose the same design state as `content.contentDocument` plus typed custom-frontend fields. The Collections UI resolves preview record URLs for hosted and render API list/item routes, persists `backy.collection-slug-policy.v1` metadata with source/fallback/update controls, computes `backy.collection-slug-policy-readiness.v1` against `:recordSlug` routes and preview records, and copies a `backy.collection-slug-policy-action-plan.v1` for custom frontend create/import forms. | Add broader cross-browser/pixel visual coverage for dynamic templates |
| Presets/templates | Starter CMS presets create sample collection plus list/detail pages | Editor component library now supports composed nested presets and includes hero, feature-grid, and lead-capture form blocks that save as normal canvas child trees. Admin reusable-section CRUD APIs persist saved canvas sections with category/tag/status filters, the editor library can save the selected element tree plus reload/insert/rename/delete active saved sections, insert synced canvas instances, refresh selected synced instances from the saved source, detach instances into independent copies, and active saved sections are discoverable through public APIs, manifest/OpenAPI, and the SDK. `/reusable-sections` now exposes a visible `backy.reusable-section-portable-pack.v1` with `backy.reusable-section-portability-readiness.v1` checks and `backy.reusable-section-portability-action-plan.v1` next actions for active library coverage, export/import readiness, frontend-template provenance, structured metadata, version restore state, synced-instance propagation, and custom frontend discovery. Site frontend-design templates persist captured page/blog/form/section/collection/product templates, create APIs can seed from `frontendDesignTemplateId`, and `GET /api/admin/sites/:siteId/templates` now exposes a normalized `backy.template-registry.v1` admin registry with type filters, content summaries, binding counts, per-template `backy.template-version.v1` readiness, registry-level `backy.template-version-readiness.v1`, a `backy.template-registry-action-plan.v1`, and clone targets for the corresponding create APIs. `/sites/:siteId` also lists captured templates with operator actions that deep-link page/blog templates into their create flows and route form/product/collection/section templates to their site-scoped template workspaces with a `frontendTemplate` search id so the selected captured template is highlighted on arrival, shows template version readiness, can prepare missing version/status/updatedAt metadata in the contract JSON, and can copy `backy.template-registry-version-action-plan.v1` for custom frontend handoff. | Add richer dynamic list/detail page presets and broader composed-template visual coverage |
...
| Live manage pages | Permissioned users can manage content from live site | Comments/forms exist, and `/api/sites/:siteId/manage/pages/:pageId` plus `/api/sites/:siteId/manage/blog/:postId` now provide public authenticated live-management bridges for page and blog post reads/edits. The routes require `pages.view` for GET, `pages.edit` for PATCH, enforce site team scope outside the nested admin URL path, delegate to the existing admin page/blog detail behavior for validation/conflicts/audit/cache invalidation, and are advertised in the public manifest, blog/pages runtime discovery, and OpenAPI endpoint maps. Hosted page and blog post routes can opt into the session-backed live management overlay with `?backyManage=1`, which hides for non-admin visitors, lets authorized editors save page title/status/homepage metadata or blog post title/status metadata with optimistic conflict protection, inspects rendered `data-backy-element-id` targets plus content-backed hidden/not-rendered elements, highlights selected rendered elements, patches simple text-like element content, button/link destinations, image source/alt/title/object-fit fields, video source/poster/playback fields, embed source/host/fullscreen/loading fields, map address/source/marker/zoom fields, top-level layer name/layout/visibility/lock fields, animation fields with custom JSON/token refs, element `actions`/`dataBindings`/`bindingSlots` JSON arrays, and common appearance props including color, border, radius, padding, margin, shadow, opacity, font family, font sizing, line height, text transform/decoration, letter spacing, and alignment inline through the matching live bridge, blocks inline content/media/appearance/action-binding edits while an element is locked, and hands the selected `elementId` into the full page or blog canvas editor. | Expand live-management routes beyond page/blog detail and keep adding richer component-specific inline workflows |
...
|---|---|---|---|
| Posts/pages | Mature CRUD with status, author, parent, slug, revisions | Admin page/blog create/edit/list flows save through backend APIs with auth/session coverage, template seeding, status/SEO settings, conflict handling, public render/manifest contracts, list-visible page/post revision summaries that deep-link into rollback history, page/blog editor revision cards that compare saved snapshots against the current saved content with visible field-level rows for title, route, status, SEO, taxonomy/media where applicable, canvas layer counts, first-pass canvas element/property rows with expandable property drill-downs, copyable revision-compare JSON briefs, page/blog `backy.*-revision-graph.v1` timeline navigation, per-node graph action/actor/status metadata, persisted revision lineage (`parentRevisionId`, `operation`, `restoreTargetRevisionId`, metadata), `backy.*-revision-branch-graph.v1` branch lanes with rollback-target edges, side-by-side visual canvas diff maps, numbered changed-element focus markers, sampled rendered-pixel comparison metrics, restore-confirmation impact previews, and actor/action metadata | Certify deeper publish snapshots and audit metadata |
...
| Revisions | Content and template revisions | Page/post revision endpoints, editor rollback panels, page/blog list summary cells, visible editor actor/action metadata, enriched page/blog `backy.*-revision-graph.v1` node metadata, persisted `backy.*-revision-branch-graph.v1` lineage/rollback edges, rendered-pixel comparison metadata, and handoff revision metadata now expose saved snapshot counts, latest notes, parent revision ids, operations, restore targets, and history deep links | Certify publish snapshots and audit metadata |
...
| Themes/global styles | Site-wide styles and per-block style supports | Shared `@backy-cms/core` compiler emits `backy.theme.v1` tokens, CSS variables, compiled stylesheet, token references, and per-block inheritance metadata into render payloads and the frontend manifest | Add richer brand-preset/theme editing UX and keep new editor style controls wired through token refs |
...
1. Canonical content contract
   ...
   - Must include elements, children, responsive overrides, style tokens, motion-tokened animations, data bindings, actions, accessibility, SEO hooks, and version metadata.
...
2. DB-backed persistence and service layer
   ...
   - Keep the configured Forms and SDK Supabase/Postgres gates in release regression paths after their 2026-05-21 certification pass.
...
- Editor: responsive overrides, layer controls, multi-select operations, composed presets, editor-level synced reusable-section instances, reusable-section instance propagation, conflict-safe page saves, grouping, keyboard shortcuts, long-session recovery, SDK add/duplicate/delete/move/resize/group/ungroup command helpers, manifest/OpenAPI `backy.editor-composition-commands.v1` discovery, and inspector-level `backy.editor-composition-readiness.v1` / `backy.editor-composition-action-plan.v1` handoff metadata are implemented and covered by focused smokes. Page editor saves send `expectedUpdatedAt`, stale saves return `PAGE_VERSION_CONFLICT`, and the editor renders a reload-latest recovery action. Remaining editor risk is continued canonical schema convergence and regression coverage as new block controls ship.
...
- Keep the configured Supabase/Postgres service-data gates in release regression paths after the Forms and SDK database-mode certification pass.
...
- `specs/page-completion-audit/backy-page-surface-audit.md`: current 41 Ready / 4 Partial / 0 Prototype / 0 Missing audit state.
