...
|---|---|---|---|
| Collections | Structured content collections independent of pages | Generic collection schema/record admin APIs, public read APIs, and a backend-backed admin `/collections` builder now exist in the runtime adapter. The builder supports schema fields, public-read status, record CRUD, record search/filter/sort controls, filtered CSV export, and dynamic route links; required/unique field validation, public visibility guards, and CSV export are covered by `test:admin-contract`. Durable DB querying is still absent. | Add DB-backed indexes/querying, CSV import, backups, audit events, full RBAC permissions, field option/reference editors, pagination UI, and bulk management |
