...
- Media upload now has runtime admin/public APIs, environment-selected local/S3/Supabase-compatible storage, quotas, signed private delivery, generated responsive WebP transform files, static upload safety checks, and admin audit logging. Production still needs editable provider configuration, full malware/provider scanning and quarantine, DB-backed version tables, and per-site ownership enforcement.
...
| Media library | Durable uploads, metadata, attachment usage | Runtime media endpoints and admin `/media` now cover upload/list/update/delete, stable-ID replacement with retained prior version metadata, folders, custom upload metadata, visibility, usage links, bulk actions, explicit page/post bind-unbind, per-asset audit activity, library usage analytics, generated responsive image manifests, Backy-served delivery analytics, static upload safety-scan state, and uploaded font registration metadata with fallback/display controls; admin upload/delete/replace/transform routes write through the active `@backy/storage` adapter while preserving stable media ids, storage paths, generated transform byte counts, and quota accounting; public media list/detail/file/transform feeds now emit cache/contract headers, ETags where applicable, conditional 304 responses for feeds, responsive image `srcSet`/variant metadata, delivery request counts for Backy-served file/transform endpoints, and media-scope cache revisions; `@backy/storage` defines local/S3/R2/Supabase upload/read/delete/sign/list/stat adapters with local smoke coverage; public render payloads expose public font assets and hosted pages inject `@font-face` rules | Add remote storage provider selection UI, full malware/provider scanning and quarantine, provider/CDN traffic and conversion analytics, and first-class DB-backed media version tables |
...
- Media: backend and UI now cover runtime storage, metadata, folders, replacement, bindings, safety-scan state, generated responsive manifests, quotas, signed/private delivery, audit activity, usage analytics, and Backy-served delivery counts; still needs editable provider setup, full malware/provider scanning, DB-backed version tables, and provider/CDN analytics.
