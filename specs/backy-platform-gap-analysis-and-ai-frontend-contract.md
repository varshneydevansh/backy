...
- Media upload now has runtime admin/public APIs, environment-selected local/S3/Supabase-compatible storage, quotas, signed private delivery, generated responsive WebP transform files, DB-backed retained-version create/list/compare/restore/delete paths with checksum/path/name/timeline diff review, static upload safety checks, optional HTTP scanner and ClamAV `clamd` scanning, quarantine controls, manual provider/CDN metric intake, and admin audit logging. Production still needs provider-side provisioning/credential rotation, visual/audio-aware binary diff previews, automated provider/CDN analytics ingestion with conversion attribution, and per-site ownership enforcement.
...
- Media: backend and UI now cover runtime storage, metadata, folders, replacement, retained-version listing/compare/restore/deletion with SHA-256/path/name/timeline deltas, bindings, safety-scan state, optional HTTP and ClamAV scanning, quarantine controls, generated responsive manifests, quotas, signed/private delivery, audit activity, usage analytics, Backy-served delivery counts, and manual provider/CDN metric recording; still needs provider-side provisioning/credential rotation, visual/audio-aware binary diff previews, and automated provider/CDN analytics ingestion with conversion attribution.
