...
| Media library | Durable uploads, metadata, attachment usage | Runtime media endpoints and admin `/media` now cover upload/list/update/delete, folders, custom upload metadata, visibility, usage links, and uploaded font registration metadata; admin upload/delete routes write through `@backy/storage` local adapter while preserving `/uploads/sites/:siteId/...` public URLs; public media list/detail feeds now emit cache/contract headers, ETags, conditional 304 responses, and media-scope cache revisions; `@backy/storage` defines local/S3/R2/Supabase upload/read/delete/sign/list/stat adapters with local smoke coverage; public render payloads expose public font assets and hosted pages inject `@font-face` rules | Add remote storage provider selection, quotas, transforms, signed/private delivery, font variant grouping, and stronger bulk/reference management |
