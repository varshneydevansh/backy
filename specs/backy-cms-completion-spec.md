...
Current audit baseline: **41 Ready / 4 Partial / 0 Prototype / 0 Missing**. The remaining Partial gates are external certification: live Settings and Commerce provider execution. Configured Forms and SDK Supabase/Postgres service-data smokes passed against a migrated disposable Postgres target on 2026-05-21. The release certification doctor can now verify saved redacted Settings/Commerce provider artifact JSON with `BACKY_SETTINGS_CERTIFICATION_ARTIFACT_PATH`, `BACKY_COMMERCE_CERTIFICATION_ARTIFACT_PATH`, and artifact-required flags before those files are used as closure evidence. Completion-status and the release doctor both expose `partialClosureReadiness` with `backy.partial-closure-readiness.v1`, making the default no-artifact provider block (`0 Ready / 4 Partial`) and artifact-accepted provider block (`4 Ready / 0 Partial`) explicit without changing the published 41/4 audit baseline. `partialClosureReadiness.auditImpact` adds `backy.partial-closure-audit-impact.v1`, so custom admin clients, the Settings UI, and the release doctor can show the artifact-backed view as **45 Ready / 0 Partial / 0 Prototype / 0 Missing** when the saved Settings and Commerce artifacts pass `npm run doctor:release-certification`; doctor output also includes `currentAuditMode` and `currentAudit` so automation can distinguish default no-artifact evidence from accepted-artifact evidence without scraping prose. The top-level release workflow and the Settings workflow's nested Commerce path now run that combined artifact-required doctor with both artifact paths before evidence upload when both provider artifacts are produced, and `npm run ci:provider-artifact-admission` gives operators the same combined artifact-required check locally after downloading both workflow artifacts. The admission helper also supports single-family modes for staged evidence: `BACKY_PROVIDER_ARTIFACT_ADMISSION_MODE=settings npm run ci:provider-artifact-admission` admits the Settings artifact for `/settings` plus Settings admin APIs, and `BACKY_PROVIDER_ARTIFACT_ADMISSION_MODE=commerce npm run ci:provider-artifact-admission` admits the Commerce artifact for `/products` plus `/orders`; the full artifact-backed audit still requires both families. The same local admission commands are exposed as `partialClosureReadiness.artifactAdmissionCommand`, `partialClosureReadiness.artifactAdmissionModes`, and row-level `artifactAdmissionCommand` through completion-status, OpenAPI/manifest, generated SDK types, and doctor output, so custom admin clients can render the final command without reading repository scripts. The same verifier metadata is exposed through completion-status `surfaceRunbooks[].artifactVerifier` so custom admin clients can show the doctor command, path env aliases, required flags, expected schema versions, `certifiedAtReady`/`artifactFreshReady` freshness checks, Commerce selected-site artifact target checks, `BACKY_PROVIDER_CERTIFICATION_ARTIFACT_MAX_AGE_HOURS` and `BACKY_PROVIDER_CERTIFICATION_ARTIFACT_FUTURE_SKEW_MINUTES` controls, and no-secret checks without scraping release docs. The doctor `partialClosureReadiness.rows[]` are now self-contained as well: every remaining Partial row carries its gate, preflight, source guard, workflow, artifact env/schema, required input family, evidence mode, artifact admission command, and next action directly on the row.
...
- In-canvas Mac zoom now installs layout-effect, non-passive native guards across window, visualViewport, document, root/body, editor shell, canvas viewport, and scaled canvas surface; recent editor-shell pointer ownership routes body/window-level browser zoom gestures back into canvas scale while keeping browser page zoom metrics stable.
...
- Better validation copy + inline field errors.
  ...
  - Commerce destructive confirmations now continue the confirm-delete hardening: `/products` single and bulk delete plus `/orders` delete expose labelled modal semantics, action-status live metadata, disabled reasons, target/count metadata, cancel buttons, and Escape recovery without relying on browser confirms.
