...
- Run live Commerce provider certification for payment, catalog, tax, shipping, discount, subscription lifecycle, provider-managed webhook paths, and the selected site's public manifest/catalog/order-contract handoffs plus `/products` provider-sync and `/orders` analytics API handoffs through `BACKY_COMMERCE_CERTIFY_SITE_ID`.
