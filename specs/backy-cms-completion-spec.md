...
- `/settings` command center now renders a source-guarded Backy completion-status panel with a copyable `backy.completion-status.v1` JSON handoff. It mirrors the public manifest/OpenAPI completion contract, reports `39 Ready / 6 Partial / 0 Prototype / 0 Missing`, lists the six remaining Partial surfaces, shows exact Forms/SDK Postgres plus Settings/Commerce provider gates, and preserves the no-secret boundary for custom admin operators.
