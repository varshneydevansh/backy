...
- **Progress**:
  ...
  - Public form submissions now record per-form notification email delivery events. In production, `BACKY_EMAIL_DELIVERY_ENDPOINT` or `BACKY_TRANSACTIONAL_EMAIL_WEBHOOK_URL` can receive the transactional email payload; without a provider endpoint, Backy records a local outbox handoff event for auditability.
  ...
  - `test:forms` starts a local webhook receiver, configures the rendered form builder, verifies email local-outbox events, verifies webhook headers/payload delivery, forces an initial failed delivery, refreshes the rendered Forms delivery panel, retries the failed delivery from the UI, and asserts failed plus retry queued/succeeded event history.
