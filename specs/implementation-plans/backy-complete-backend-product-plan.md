...
- **High / patched, smoke-covered**: Canvas elements select but do not drag reliably in the current editor runtime.
  ...
  - Validation added: `npm run test:editor-drag --workspace @backy-cms/admin` covers text/image/container/nested-child/form drag, image/form resize, backend save persistence, and fresh editor reload rehydration. Remaining validation: undo after drag, keyboard nudging, and broader manual browser QA across zoom/breakpoints.
