...
18. `/orders`: backend-backed private order operations workspace with schema setup/sync, private order permissions, public checkout-intake contract, payment/fulfillment/tracking/refund/address fields, configured order notification delivery, CSV/handoff export, and smoke coverage through `test:commerce`; remaining work is provider execution, fulfillment integrations, fraud/risk controls, and production provider workflows
