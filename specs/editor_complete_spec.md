...
- `BACKY_EDITOR_LAYERS_SMOKE=1 npm run test:editor-drag --workspace @backy-cms/admin` covers panel opening, nested depth, click and keyboard collapse/expand, nested layer search/filter behavior, inline rename persistence, Ctrl/Cmd multi-select, Shift range-select, roving-focus keyboard row selection/navigation tree semantics, selected-row action visibility, hidden row action interactivity guards, drag reorder, hide, lock, single and multi duplicate/delete, manual save, and persisted layer state.
