...
- `BACKY_EDITOR_LAYERS_SMOKE=1 npm run test:editor-drag --workspace @backy-cms/admin` covers panel opening, nested depth, Ctrl/Cmd multi-select, Shift range-select, selected-row action visibility, hidden row action interactivity guards, drag reorder, hide, lock, single and multi duplicate/delete, manual save, and persisted layer state.
