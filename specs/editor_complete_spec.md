...
- ⚠️ Page templates
  ...
  - Blog/content presets now carry root and child binding-slot metadata for post title, excerpt, featured image, link, category/taxonomy, and collection-record targets, while live `dataBindings` remain attached only after an author selects a real collection/source.
