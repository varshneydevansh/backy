...
| codeBlock | ✅ code/language/filename/caption/line numbers/wrap/copy metadata | ✅ | ✅ | Static technical snippets render as safe text, preserve whitespace, expose `data-backy-code-*` metadata, and are advertised in the custom-frontend component API contract | ✅ |
...
- The bottom grid/snap and zoom HUD controls expose shared live action statuses, command ids, ready/selected state, disabled reasons, and `aria-describedby` wiring, so custom shells can tell active canvas controls apart from unavailable ones without scraping visual classes.
- Mac/browser zoom is guarded inside the editor canvas: Cmd/Ctrl wheel, visualViewport/root/body/shell/viewport/surface `mousewheel`, keyboard zoom, and gesture pinch paths update canvas scale while keeping browser page zoom stable; coordinate-less gestures from outside the editor shell pass through without changing canvas zoom, and the scaled canvas surface plus recent editor-shell pointer fallback carry native non-passive capture coverage.
...
- Hardened in-canvas Mac zoom by listening for modern `wheel`, legacy visualViewport/root/body/shell/viewport/surface `mousewheel`, keyboard zoom, and gesture pinch events with layout-effect non-passive capture guards; the rendered zoom smoke now proves the legacy root `mousewheel` path and a recent-pointer body-level Mac wheel path change only the canvas scale, keep browser page zoom metrics stable, leave coordinate-less header/sidebar gestures outside the editor shell uncancelled, and verify the scaled canvas surface advertises `native-capture` zoom handling.
...
- Added command-backed action-status coverage for the bottom canvas grid/snap and zoom HUDs, including selected toggle state, direct zoom slider status, shared live regions, and command ids verified by focused zoom and grid/snap browser smokes.
...
- The Site detail Frontend handoff panel and the canvas editor Composition handoff panel now expose a copyable `backy.custom-frontend-agent-handoff.v1` agent block with docs, direct `/agent-handoff`, manifest/OpenAPI/render/frontend-design endpoints, `apiAlignment`, design-state preserve fields, and page/blog canvas entry points for Backy-canvas or custom-frontend template sources.
...
- The same public handoff now includes `apiAlignment` (`backy.custom-frontend-api-alignment.v1`) so AI/custom frontend agents can read one typed block for the direct read-start endpoint, manifest/OpenAPI mirrors, SDK/generated types, public discovery endpoints, admin write boundary, creation routes, preserved design fields, and render/resolve verification before generating a custom frontend.
...
- ⚠️ Page templates
  ...
  - Backend reusable-section APIs now persist saved canvas section patterns, the editor library can load active saved sections, save the selected element tree with reachable inline name validation, insert saved sections as synced canvas instances, reflow later root layers for single-root and multi-root section drops, refresh a selected synced instance from its saved source, detach an instance into an independent editable copy, and expose active sections through public APIs, manifest/OpenAPI, and the SDK.
  ...
  - Page-create template selection now exposes a shared `page-template-selection-action-status` live contract plus per-control ready/selected/busy/blocked metadata for the Backy-canvas/custom-frontend source buttons, captured frontend template cards, category filters, and starter template options. Rendered page-create smoke verifies the Backy canvas action metadata, the custom frontend template selection metadata, and a landing starter backend/render pass with `BACKY_PAGE_CREATE_STARTER_FILTER=landing`.
  - Blog-create template selection now exposes the matching `blog-template-selection-action-status` live contract plus ready/selected/busy/blocked metadata for the Backy-canvas source, custom-frontend source, and captured frontend blog template cards. Rendered blog-create smoke verifies the custom frontend template card/source metadata, template provenance, autosave recovery, focused canvas boot, responsive overrides, preview creation, and persisted custom frontend design metadata.
  - Products now expose the matching `products-frontend-template-action-status` live contract for captured custom-frontend product templates, including ready/busy/blocked metadata, disabled reasons, target template/site ids, and create/copy action status so external storefront template seeding is inspectable before it creates catalog records.
  - Forms now expose the matching `forms-frontend-template-action-status` live contract for captured custom-frontend form templates, including ready/busy/blocked metadata, disabled reasons, target template/site ids, and create/copy action status so custom frontend intake forms are inspectable before they create backend form records.
  - Collections now expose the matching `collections-frontend-template-action-status` live contract for captured custom-frontend collection templates, including ready/busy/blocked metadata, disabled reasons, target template/site ids, and create/copy action status so custom frontend dataset schemas are inspectable before they create CMS collections.
