...
**Remaining parity**
- Continue expanding rich-text table/list edge-case coverage and responsive breakpoint pixel QA across remaining component combinations such as comments, repeaters, embeds, interactive components, and choice inputs. Long-session editor stress coverage now has a focused smoke path through `npm run test:editor-stress --workspace @backy-cms/admin`.
