...
- **Current progress**:
  ...
  - `ComponentLibraryItem` and nested preset children now support binding-slot metadata, and blog/content presets seed CMS field targets for titles, excerpts, images, meta/category labels, URLs, taxonomy, and collection records without faking live `dataBindings`.
  - The editor Data panel now renders those preset binding slots, applies matching selected-collection fields as real `dataBindings`, can apply matching child slots across a composed card/section in one history step, lets named root slots target child repeaters, routes repeater slots into collection/field props, and the shared content/render-payload contracts preserve the slot metadata for custom frontend handoff.
  - The editor catalog includes composed hero, feature-grid, lead-capture form, blog post card, latest posts, category-list, and related-content presets, and canvas drop paths instantiate those presets as normal nested editable elements; latest posts, category-list, and related-content now seed collection-ready repeaters with root and field binding slots.
  - Public repeaters can now render authored child templates once per hydrated record and resolve child collection `dataBindings` from the active record, preserving the automatic card fallback for simple dynamic list blocks.
  - Shared starter-page chrome now keeps responsive content offsets, breakpoint-specific footer placement, and header/footer navigation geometry aligned when Backy injects editable site chrome into starter templates.
  - The built-in landing starter now seeds authored tablet/mobile responsive overrides for its hero, CTA, media placeholder, and feature cards, matching the starter-page parity path used by commerce and content templates.
  - The built-in storefront starter now seeds authored tablet/mobile responsive overrides for its commerce hero, featured product, and catalog cards, matching the newer commerce starter-page parity path.
