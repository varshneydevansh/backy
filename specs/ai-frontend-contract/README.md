...
It returns the `content-payload.schema.json` shape for pages, blog posts, and collection dynamic item routes from the current public data adapter. Dynamic collection render payloads include route resource metadata and can carry the same rich design envelope as Backy's editor captures: `content.customCSS`, `content.customJS`, `content.contentDocument`, `content.themeTokenRefs`, `content.assets`, `content.animations`, `content.interactions`, `content.dataBindings`, `content.editableMap`, `content.seo`, and `content.metadata`. Product-backed dynamic item routes include `route.resource.designReadiness` using `backy.product-design-readiness.v1` so custom storefronts can detect editable product template/content/animation readiness from `/render`. Database-backed repository mode and local contract smokes are implemented for the current public surface, and `BACKY_DATABASE_DISPOSABLE_CONFIRMED=true npm run ci:sdk-postgres-smoke` passed against a migrated disposable Postgres service on 2026-05-21, moving the SDK/database-mode contract to Ready.

`npm run test:admin-contract --workspace @backy/public` validates the page, blog post, collection-bound page, and collection dynamic item render responses against `content-payload.schema.json` so contract drift is caught during the public API smoke pass. The same smoke verifies that `/render` exposes the dataset manifest, resolved collection fields/records, normalized element binding, collection-record editable map entry, uploaded public font asset manifests, and the typed rich design-state fields custom frontends need to reopen authored dynamic pages.
...
It returns `frontend-manifest.schema.json` and gives custom frontends a single bootstrap document for site identity, compiled theme tokens, public endpoint URLs, route patterns, localized route-pattern variants, module capabilities, configured navigation, routing/redirect metadata, page/blog/collection/reusable-section/form/media summaries, collection field schemas, form submit URLs, and form-to-collection targets. This mirrors the role of REST discovery in a WordPress-like frontend integration while keeping Backy's render payload as the page/post/item-specific contract.

`data.modules.theme` carries structured `backy.theme-discovery.v1` metadata for custom frontend styling. It includes the compiled `backy.theme.v1` token contract, ready-to-apply `--backy-*` CSS variables, a compiled stylesheet, token reference map, root/scoped selectors, element-level `tokenRefs` plus document-level `themeTokenRefs` inheritance metadata, editable Backy theme fields, and capability flags for typography families, spacing scale, custom CSS, live editing, frontend-design overrides, token references, animation motion-token refs, and stylesheets.

`data.modules.liveManagement` carries structured `backy.live-management.v1` metadata for authenticated inline editing from a custom frontend. It advertises the `GET/PATCH /api/sites/:siteId/manage/pages/:pageId` and `GET/PATCH /api/sites/:siteId/manage/blog/:postId` bridges plus the authenticated page/blog lifecycle route templates for list, create, detail/update/delete, publish/archive, preview token creation, revisions, and rollback. Those live bridge responses are private/no-store and expose `x-backy-contract-version`, `x-backy-schema-version`, `x-backy-cache-scope`, `x-backy-request-id`, `x-backy-site-id`, and `x-backy-live-management-resource`; page bridges use `backy.live-management-page.v1`, and blog bridges use `backy.live-management-blog-post.v1`. The `backy.content-lifecycle-commands.v1` handoff names the SDK helpers such as `createAdminPage()` and `createAdminBlogPost()`, the `frontendDesignTemplateId` clone field, session/API-key auth headers, required permissions, site-scope enforcement, editable target paths, optimistic `expectedUpdatedAt` conflict handling, and stable page/blog conflict codes including `PAGE_VERSION_CONFLICT` and `BLOG_VERSION_CONFLICT`. Editable targets include durable media/font/design state such as `props.mediaId`, `props.fontMediaId`, `props.imageFocalPoint`, `assetIds`, `tokenRefs.*`, `animation.type`, `animation.from`, `animation.to`, `animation.tokenRefs.*`, and responsive override paths like `responsive.mobile.x`, `responsive.tablet.width`, `responsive.*.props.*`, `responsive.*.styles.*`, and `responsive.*.tokenRefs.*`, so custom frontend editors can round-trip central assets, animations, breakpoint layout, and per-breakpoint style choices instead of falling back to URL-only patches. SDK patch helpers can write those responsive paths directly or through editable-map `targetPath` aliases, and they keep element `assetIds` synchronized when an external editor changes media/font asset paths, including responsive, fallback, poster, and background media IDs.

`data.modules.pagesRuntime` carries structured `backy.pages-discovery.v1` metadata for custom frontend page routing, public page reads, preview-token reads, render payloads, and live-management handoff. It advertises page list/detail/resolve/render/live-manage endpoint templates, authenticated page lifecycle route templates, GET/PATCH/POST/DELETE methods, public page paths, published/scheduled counts, SEO/frontend-design capabilities, page creation/template-cloning/readiness/publish/archive/revision capabilities, public/private cache boundaries, preview-token privacy rules, query filters, and stable page error markers. Its `managementPolicy` exposes the `backy.pages-management.v1` route, permission, SDK helper, and design-state preservation contract for external page builders.

`data.modules.blogRuntime` carries structured `backy.blog-discovery.v1` metadata for custom frontend blog indexes, post detail pages, taxonomy/archive/search filters, RSS feeds, preview-token reads, render payloads, live-management handoff, and route resolution. It advertises blog list/detail/live-manage/RSS/category/tag/author/resolve/render endpoint templates, authenticated post lifecycle route templates, GET/PATCH/POST/DELETE methods, public post paths, published/scheduled and taxonomy/feed counts, post creation/template-cloning/readiness/publish/archive/revision capabilities, public/private cache boundaries, preview-token privacy rules, supported query filters, and stable blog error markers. Its `managementPolicy` exposes the `backy.blog-management.v1` route, permission, SDK helper, and design-state preservation contract for external blog editors.

`data.modules.collectionsRuntime` carries structured `backy.collections-discovery.v1` metadata for custom frontend collection schemas, dynamic list/detail routes, public record reads, and visitor writes. It advertises collection/record endpoint templates, GET/POST/PATCH/DELETE methods, public read/create/update/delete counts, supported field types, cache and privacy boundaries, draft-create/write-token policy, visitor write-policy metadata, and stable validation/slug-conflict error markers.

`data.modules.reusableSectionsRuntime` carries structured `backy.reusable-sections-discovery.v1` metadata for custom frontend section browsers and reusable page-builder blocks. It advertises list/detail endpoint templates, GET methods, active-only public read policy, category/tag/search filters, canvas-content support, public discovery cache policy, section/content schema markers, and the stable `REUSABLE_SECTION_NOT_FOUND` error code.

`data.modules.formsRuntime` carries structured `backy.forms-discovery.v1` metadata for custom frontend form rendering and submission. It advertises list/detail/definition/submit/submission/contact endpoint templates, methods, active and collection-target counts, moderation modes, validation and contact-share capabilities, public/private cache boundaries, and privacy rules showing that public definitions exclude submission/contact data.

`data.modules.commerceRuntime` carries structured `backy.commerce-discovery.v1` metadata for custom storefronts, product catalog/detail pages, checkout/order-contract flows, provider webhooks, inventory reservations, pricing rules, and provider certification. It advertises catalog/detail/order/webhook endpoint templates, GET/POST methods, product and private-order collection boundaries, product filters/facets, public/private cache policy, customer-data privacy rules, and stable commerce error markers. Public commerce catalog and order-contract GET responses emit Backy contract/cache headers, `x-backy-schema-version`, ETags where cacheable, and database-mode `content` cache revisions; tokenized order-status handoff and checkout POST responses remain private/no-store because they can carry customer/order-session data.
