...
`data.modules.liveManagement` carries structured `backy.live-management.v1` metadata for authenticated inline editing from a custom frontend. It advertises the `GET/PATCH /api/sites/:siteId/manage/pages/:pageId` bridge, accepted session/API-key auth headers, required `pages.view` and `pages.edit` permissions, site-scope enforcement, editable target paths, optimistic `expectedUpdatedAt` conflict handling, and the stable `PAGE_VERSION_CONFLICT` error code.
