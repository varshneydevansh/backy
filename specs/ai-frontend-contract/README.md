...
`data.modules.commerceRuntime` carries structured `backy.commerce-discovery.v1` metadata for custom storefronts, product catalog/detail pages, checkout/order-contract flows, provider webhooks, inventory reservations, pricing rules, and provider certification. It advertises catalog/detail/order/webhook endpoint templates, GET/POST methods, product and private-order collection boundaries, product filters/facets, public/private cache policy, customer-data privacy rules, and stable commerce error markers.
