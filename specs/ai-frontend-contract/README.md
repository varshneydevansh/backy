...
Admin media detail can replace the stored file for an existing media item with `POST /api/admin/sites/:siteId/media/:mediaId` while keeping the media id stable for custom frontends. Replacements retain prior file metadata under `metadata.replacementVersions`, count retained versions against quota usage, and emit `media.replace` audit events. Retained versions can be removed with `DELETE /api/admin/sites/:siteId/media/:mediaId/versions/:versionId`, which deletes the stored prior file when a storage path is recorded and emits `media.version.delete`.
