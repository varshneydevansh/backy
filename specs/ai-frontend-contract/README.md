...
`packages/sdk-js` provides the current TypeScript client for custom frontends. It uses only public Backy APIs and exposes helpers for site discovery, manifest/OpenAPI bootstrap, frontend-design contract reads, route resolution, page/blog reads, render payloads, SEO discovery, navigation, media, media file URL construction, collections/records, reusable sections, commerce catalog/order intake, forms/submissions/contacts, comments/reports, and interaction events. Its exported types now cover the main public contract objects (`BackyFrontendDesignContract`, `BackyRenderPayload`, typed route results including redirect/gone routes, content documents/elements, page/post resources, SEO routes, media/font assets and manifests, collection schemas/records, commerce catalog/products/orders, reusable sections, form submissions/contacts, comments, events, response metadata, and conditional 304 results) so frontends can consume Backy with stronger compile-time guarantees. `manifestCached`, `frontendDesignCached`, `openapiCached`, `renderCached`, `pagesCached`, `blogCached`, `navigationCached`, `seoCached`, `mediaCached`, `mediaAssetCached`, `mediaFontsCached`, `collectionsCached`, `collectionCached`, `reusableSectionsCached`, `reusableSectionCached`, `recordsCached`, and `commerceCatalogCached` expose cache/contract headers, including `x-backy-cache-revision`, and send `If-None-Match` when an ETag is provided, letting custom frontends reuse cached discovery/frontend-design/render/page/blog/navigation/SEO/media/font/collection-schema/reusable-section/dynamic-data/product-catalog payloads without dropping to raw `fetch`. The SDK `resolve()` helper returns a normal route envelope for pages/posts/dynamic routes/redirects and returns a non-throwing gone envelope for HTTP 410 routes; ordinary 404 and other errors still throw `BackyApiError`. The smoke command `npm run test:smoke --workspace @backy/sdk-js` validates read flows, SDK ETag/304 helpers, frontend-design contract reads, page/blog reads, SEO discovery, reusable-section reads, collection schema reads, redirect/gone route handling, media file URL construction, optional commerce catalog reads where a catalog exists, and SDK public writes for collection records, form submissions, form contacts, page comments, comment moderation, comment reports, and interaction events against a temporary site, building confidence that a frontend can consume Backy with stronger compile-time guarantees without importing admin/editor code.
...
Admin media detail can replace the stored file for an existing media item with `POST /api/admin/sites/:siteId/media/:mediaId` while keeping the media id stable for custom frontends. Replacements retain prior file metadata under `metadata.replacementVersions`, count retained versions against quota usage, and emit `media.replace` audit events.
