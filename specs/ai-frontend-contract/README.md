...
`data.modules.theme` carries structured `backy.theme-discovery.v1` metadata for custom frontend styling. It includes the compiled `backy.theme.v1` token contract, ready-to-apply `--backy-*` CSS variables, root/scoped selectors, editable Backy theme fields, and capability flags for typography families, spacing scale, custom CSS, live editing, and frontend-design overrides.

`data.modules.liveManagement` carries structured `backy.live-management.v1` metadata for authenticated inline editing from a custom frontend. It advertises the `GET/PATCH /api/sites/:siteId/manage/pages/:pageId` and `GET/PATCH /api/sites/:siteId/manage/blog/:postId` bridges, accepted session/API-key auth headers, required `pages.view` and `pages.edit` permissions, site-scope enforcement, editable target paths, optimistic `expectedUpdatedAt` conflict handling, and stable page/blog conflict codes including `PAGE_VERSION_CONFLICT` and `BLOG_VERSION_CONFLICT`.

`data.modules.pagesRuntime` carries structured `backy.pages-discovery.v1` metadata for custom frontend page routing, public page reads, preview-token reads, render payloads, and live-management handoff. It advertises page list/detail/resolve/render/live-manage endpoint templates, GET/PATCH methods, public page paths, published/scheduled counts, SEO/frontend-design capabilities, public/private cache boundaries, preview-token privacy rules, query filters, and stable page error markers.

`data.modules.blogRuntime` carries structured `backy.blog-discovery.v1` metadata for custom frontend blog indexes, post detail pages, taxonomy/archive/search filters, RSS feeds, preview-token reads, render payloads, live-management handoff, and route resolution. It advertises blog list/detail/live-manage/RSS/category/tag/author/resolve/render endpoint templates, GET/PATCH live-management methods, public post paths, published/scheduled and taxonomy/feed counts, public/private cache boundaries, preview-token privacy rules, supported query filters, and stable blog error markers.
