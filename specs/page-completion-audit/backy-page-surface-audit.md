...
| `/media` | Partial | `apps/admin/src/routes/media.tsx` now loads/uploads/updates/deletes through backend media endpoints, with metadata editing, search/type/visibility filters, and folder create/delete/filter/assignment UI. | Add private media enforcement, image transforms, storage provider abstraction, quota checks, full font registration workflow, and reference tracking. |
