...
| `/pages/$pageId/edit` | Partial | `apps/admin/src/routes/pages.$pageId.edit.tsx` now loads/saves through `GET/PATCH /api/admin/sites/:siteId/pages/:pageId`, exposes backend-backed publish/archive/revision rollback controls, and hydrates site media so uploaded fonts are available in editor font controls. | Add preview URL, stronger autosave status, responsive breakpoints, layers/alignment/snapping/grouping, keyboard shortcuts, robust undo/redo, and complete component properties. |
...
4. **Publishing model**: wire publish/revision controls into admin pages, then add preview tokens and cache invalidation.
