...
| `/pages/$pageId/edit` | Partial | `apps/admin/src/routes/pages.$pageId.edit.tsx` now loads/saves through `GET/PATCH /api/admin/sites/:siteId/pages/:pageId`, exposes backend-backed publish/archive/revision rollback controls, and hydrates site media so uploaded fonts are available in editor font controls. Backend preview tokens now allow draft page render/page API access. | Wire preview URL into the editor UI, stronger autosave status, responsive breakpoints, layers/alignment/snapping/grouping, keyboard shortcuts, robust undo/redo, and complete component properties. |
...
| `/blog/$postId` | Partial | `apps/admin/src/routes/blog.$postId.tsx` now loads/saves/deletes through `GET/PATCH/DELETE /api/admin/sites/:siteId/blog/:postId` and exposes backend-backed publish/archive/revision rollback controls. Backend preview tokens now allow draft post API access. | Wire preview URL into the editor UI, schedule controls, categories/tags, featured media, canonical/SEO controls, and comments/moderation status. |
...
4. **Publishing model**: wire preview links into admin pages, then add cache invalidation and audit events.
