...
| `GET /api/sites/:siteId/render` | Partial | `apps/public/src/app/api/sites/[siteId]/render/route.ts` returns the new AI/custom-frontend render payload and supports token-gated draft page preview. | Schema validation tests, version negotiation, post/dynamic route coverage, cache strategy, stable SDK examples, and token auditing. |
...
4. **Publishing model**: add cache invalidation, preview token audit events, and contract tests for hosted previews.
