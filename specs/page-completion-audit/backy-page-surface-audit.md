...
| `/media` | Partial | `apps/admin/src/routes/media.tsx` now loads/uploads/updates/deletes through backend media endpoints, with metadata editing and search/type/visibility filters. | Add folders, private media enforcement, image transforms, storage provider abstraction, quota checks, font registration, and reference tracking. |
...
| `GET/POST/PATCH/DELETE /api/admin/sites/:siteId/media` | Partial | Admin media upload/list/update/delete endpoints store files under `public/uploads` and metadata under `data/backy/media-library.json`; list filters support type, visibility, search, and tag; the `/media` UI calls these endpoints. | Auth/RBAC, durable database, object storage, virus/type scanning, quotas, background transforms, folders, and audit events. |
