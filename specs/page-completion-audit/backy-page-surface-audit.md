...
| `/sites` | Prototype | `apps/admin/src/routes/sites.tsx` lists local sites with search/sort/delete. | Admin site list API, pagination, filters, ownership/RBAC, domain/publish status, duplicate/archive flows, and destructive-action confirmation UX. |
| `/sites/new` | Prototype | Local create flow exists. | Backend site provisioning, template/site type selection, slug/domain validation, theme seed, default pages, publish target configuration, and failure recovery. |
| `/sites/$siteId` | Partial | `apps/admin/src/routes/sites.$siteId.tsx` edits local site fields and has backend-connected forms/comments moderation sections. | Persist site settings through admin APIs, domain verification, SEO/theme/publish settings, webhook configuration, form builder management, comment policy settings, and audit logs. |
| `/pages` | Prototype | `apps/admin/src/routes/pages.tsx` lists local pages. | Backend page list CRUD, hierarchy/navigation management, published/draft/scheduled filters, route conflict checks, bulk actions, revisions, and public preview links. |
| `/pages/new` | Prototype | `apps/admin/src/routes/pages.new.tsx` creates a local page and has template choices. | Backend create endpoint, template application, route/slug validation, site-aware navigation placement, SEO defaults, and immediate transition into editor. |
| `/pages/$pageId/edit` | Partial | `apps/admin/src/routes/pages.$pageId.edit.tsx` uses `CanvasEditor`; recent fixes made canvas selection/drag behavior usable and save serializes to local store. | Backend save/publish APIs, autosave, revision history, preview URL, responsive breakpoints, layers/alignment/snapping/grouping, keyboard shortcuts, robust undo/redo, and complete component properties. |
