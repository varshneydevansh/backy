...
| `/media` | Partial | `apps/admin/src/routes/media.tsx` now loads/uploads/updates/deletes through backend media endpoints, with metadata editing and search/type/visibility filters. Backend media folder endpoints exist. | Wire folder UI, then add private media enforcement, image transforms, storage provider abstraction, quota checks, font registration, and reference tracking. |
