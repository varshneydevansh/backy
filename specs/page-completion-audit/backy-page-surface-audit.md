...
| `/pages/$pageId/edit` | Partial | `apps/admin/src/routes/pages.$pageId.edit.tsx` now loads and saves through `GET/PATCH /api/admin/sites/:siteId/pages/:pageId`, with local fallback state. Backend page publish/archive/rollback/revisions endpoints exist. | Wire revision history and publish/archive/rollback controls into the editor UI, then add preview URL, stronger autosave status, responsive breakpoints, layers/alignment/snapping/grouping, keyboard shortcuts, robust undo/redo, and complete component properties. |
...
| `/blog/$postId` | Partial | `apps/admin/src/routes/blog.$postId.tsx` now loads, saves, and deletes through `GET/PATCH/DELETE /api/admin/sites/:siteId/blog/:postId`, with local fallback state. Backend post publish/archive/rollback/revisions endpoints exist. | Wire revision history and publish/archive/rollback controls into the blog UI, then add schedule controls, categories/tags, featured media, canonical/SEO controls, preview, and comments/moderation status. |
