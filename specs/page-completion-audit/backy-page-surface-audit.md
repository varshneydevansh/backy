...
| `/sites` | Partial | `apps/admin/src/routes/sites.tsx` still lists local sites, but backend endpoints now exist at `GET/POST /api/admin/sites`. | Wire UI to admin API, add filters, ownership/RBAC, domain/publish status, duplicate/archive flows, and destructive-action confirmation UX. |
| `/sites/new` | Partial | Local create flow exists and `POST /api/admin/sites` now validates name/slug and persists to `data/backy/admin-content.json`. | Wire UI to API, add template/site type selection, domain validation, default pages, publish target configuration, and failure recovery. |
| `/sites/$siteId` | Partial | `apps/admin/src/routes/sites.$siteId.tsx` edits local site fields; `GET/PATCH/DELETE /api/admin/sites/:siteId` now exists; forms/comments moderation sections are backend-connected. | Wire site settings UI to admin APIs, add domain verification, SEO/theme/publish settings, webhook configuration, form builder management, comment policy settings, and audit logs. |
| `/pages` | Partial | `apps/admin/src/routes/pages.tsx` still lists local pages, but backend endpoints now exist at `GET/POST /api/admin/sites/:siteId/pages`. | Wire UI to admin API, add hierarchy/navigation management, published/draft/scheduled filters, route conflict checks, bulk actions, revisions, and public preview links. |
| `/pages/new` | Partial | Local create flow exists and `POST /api/admin/sites/:siteId/pages` now validates title/slug and persists page content/meta. | Wire UI to API, apply templates, add site-aware navigation placement, SEO defaults, and immediate transition into editor. |
| `/pages/$pageId/edit` | Partial | `apps/admin/src/routes/pages.$pageId.edit.tsx` uses `CanvasEditor`; backend detail/update/delete endpoints now exist at `/api/admin/sites/:siteId/pages/:pageId`, but the route still saves to local store. | Wire editor save/publish to backend APIs, add autosave, revision history, preview URL, responsive breakpoints, layers/alignment/snapping/grouping, keyboard shortcuts, robust undo/redo, and complete component properties. |
| `/blog` | Prototype | `apps/admin/src/routes/blog.tsx` lists local posts. | Backend post list CRUD, categories/tags/authors, scheduled posts, SEO metadata, bulk actions, comments status, and preview/published URLs. |
| `/blog/new` | Partial | `apps/admin/src/routes/blog.new.tsx` uses `CanvasEditor` for post body and saves locally. | Backend post create/publish, category/tag assignment, featured image, excerpt/SEO validation, autosave/revisions, preview, and block/editor defaults suited to long-form writing. |
| `/blog/$postId` | Partial | `apps/admin/src/routes/blog.$postId.tsx` loads saved canvas content and edits local posts. | Backend update/delete, revisions, schedule/publish flow, categories/tags, featured media, canonical/SEO controls, preview, and comments/moderation status. |
