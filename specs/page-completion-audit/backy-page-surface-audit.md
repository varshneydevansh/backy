...
| `/media` | Partial | `apps/admin/src/routes/media.tsx` now loads/uploads/updates/deletes through backend media endpoints, with metadata editing, search/type/visibility filters, and folder create/delete/filter/assignment UI. Page/post save, rollback, and delete flows sync media usage references. Public media catalog hides private items. | Add authenticated/signed private media file delivery, image transforms, storage provider abstraction, quota checks, full font registration workflow, and richer reference browsing UI. |
