...
| `/blog/$postId` | Partial | `apps/admin/src/routes/blog.$postId.tsx` now loads/saves/deletes through `GET/PATCH/DELETE /api/admin/sites/:siteId/blog/:postId` and exposes backend-backed publish/archive/revision rollback/preview controls. Backend preview tokens allow draft post API access. | Add schedule controls, categories/tags, featured media, canonical/SEO controls, hosted post preview rendering, and comments/moderation status. |
...
| Public hosted pages | Partial | `apps/public/src/app/sites/[subdomain]/[[...path]]/page.tsx` renders content and supports token-gated draft page previews with preview metadata marked noindex. | Full route resolution, 404/redirects, sitemap/robots, theme inheritance, responsive rendering guarantees, deployment/cache policy, and hosted blog post rendering. |
...
4. **Publishing model**: add cache invalidation, preview token audit events, and hosted post preview rendering.
