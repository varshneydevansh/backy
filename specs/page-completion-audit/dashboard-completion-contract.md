...
- Deployment health: implemented for the current Settings/runtime slice. The dashboard now renders Vercel project metadata status, deploy-token readiness, environment, auto/preview deploy modes, latest persisted or session deployment check, domain presence, cache invalidation/rebuild audit or preflight evidence, and the same non-secret deployment summary in the frontend handoff JSON.
