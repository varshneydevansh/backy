...
2. `GET /api/sites/:siteId/agent-handoff`
   ...
   - Returns `backy.custom-frontend-agent-handoff-response.v1` with the canonical `agentBrief`, `handoff`, `readStart`, `apiAlignment`, `componentApiContract`, `routing`, `deploymentTopology`, `canvasFirst`, `contentCreation`, and `designState` blocks.
...
`componentTypeContracts` is the per-element lookup table for generated frontends. It names the Backy element `type`, UI family, readable/writable `propPaths`, style paths, responsive override paths, and binding paths for headings, text, images, buttons, navigation, forms, repeaters, comments, embeds, static code snippets, interactive figures, and sandboxed code components. Treat this table as the source of truth when mapping Wix/Webflow-style components into Backy JSON: render from the advertised props, write edits back to the same paths, and preserve any unknown keys for forward compatibility.

Every type contract also includes `creationHints`: `defaultSize`, `defaultProps`, `propsTemplate`, `requiredFields`, and a `minimalElement` payload. Custom frontend agents should create new Backy elements by cloning `creationHints.minimalElement`, replacing the stable `id`, `x`/`y`, dimensions, props, styles, responsive overrides, bindings, and children as needed, then preserving unknown keys during later edits. This is the machine-readable creation path for API-first builders that need to add headings, sections, navigation, forms, code blocks, or reusable layout pieces without scraping the admin editor catalog.
...
Agents should also read `customFrontendAgentHandoff.deploymentTopology` before deploying Backy or a website that uses Backy. It has schema `backy.deployment-topology.v1`.
...
- Blog posts inherit the page template vocabulary through `contentCreation.blogChildStarterTemplates` and `contentCreation.blogTemplateInheritance` (`backy.blog-child-template-inheritance.v1`). Each child starter advertises `starterTemplate`, `templateSource`, `frontendDesignTemplateId`, a Backy-canvas route, and a custom-frontend route. Use this catalog for article, investigation, media/audio transcript, newsletter issue, and case-study authoring so the post opens in the canvas with the selected site design instead of a blank one-off.
...
- Subscriber payloads expose `subscriptionStatus` as the simple audience state (`subscribed`/`unsubscribed`) and `newsletterStatus` as the full operational lifecycle (`subscribed`, `unsubscribed`, `pending`, `bounced`, `complained`) for provider workers and moderation dashboards.
- Copy the workspace handoff (`backy.newsletter-management-handoff.v1`) when an AI/frontend agent or email-sync worker needs the canonical `/agent-handoff`, manifest, OpenAPI, form definition URLs, submit URLs, private management endpoints, canvas routes, and no-secret delivery boundary.
