...
- `PATCH /api/sites/:siteId/manage/blog/:postId`
  ...
  - Custom frontends using the SDK can call `listBackyContentElements()` to build a flat external-editor layer inventory with parent/depth/path metadata and editable target paths, then call `findBackyContentElement()`, `patchBackyContentElement()`, `patchBackyContentElements()`, `patchBackyContentEditableField()`, `patchBackyContentEditableFields()`, `patchBackyContentEditableMapEntry()`, and `patchBackyContentEditableMapEntries()` to apply one or many advertised editable target paths, render `editableMap`-style `{ elementId, field, value }` updates, or keyed editable-map updates such as `hero.title -> props.content`, `button.primary.color -> styles.color`, `layout.x`, and `visibility.locked` to nested page/blog content before sending the full live-management update body with `expectedUpdatedAt`.
