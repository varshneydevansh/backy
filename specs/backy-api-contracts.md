...
- `GET /api/admin/settings`
  ...
  - The SDK exposes `adminSettings()` and `updateAdminSettings()` with the same per-call admin auth options as live-management methods so custom admin clients can read and update workspace Settings while receiving the provider-certification and frontend-database-certification handoffs as typed envelopes.
