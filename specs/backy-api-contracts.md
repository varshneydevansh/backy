...
- `PATCH /api/sites/:siteId/manage/blog/:postId`
  ...
  - Custom frontends using the SDK can call `findBackyContentElement()`, `patchBackyContentElement()`, and `patchBackyContentElements()` to apply one or many advertised editable target paths such as `props.content`, `styles.color`, `layout.x`, and `visibility.locked` to nested page/blog content before sending the full live-management update body with `expectedUpdatedAt`.
