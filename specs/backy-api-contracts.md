...
- `PATCH /api/sites/:siteId/manage/blog/:postId`
  ...
  - Custom frontends using the SDK can call `listBackyContentElements()` to build a flat external-editor layer inventory with parent/depth/path metadata and editable target paths, then call `findBackyContentElement()`, `patchBackyContentElement()`, `patchBackyContentElements()`, `patchBackyContentEditableField()`, `patchBackyContentEditableFields()`, `patchBackyContentEditableMapEntry()`, `patchBackyContentEditableMapEntries()`, and `patchBackyContentEditableMapValues()` to apply one or many advertised editable target paths, render `editableMap`-style `{ elementId, field, value }` updates, keyed editable-map updates such as `hero.title -> props.content`, or form-state objects such as `{ "button.primary.color": "#111827" }` to nested page/blog content. `buildBackyLiveManagedPageEditableMapUpdate()` and `buildBackyLiveManagedBlogPostEditableMapUpdate()` package those patched content trees into full live-management update bodies with `expectedUpdatedAt`.
