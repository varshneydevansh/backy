...
- `GET /api/admin/sites/:siteId/forms`
  ...
  - The SDK exposes `adminForms()`, `createAdminForm()`, `adminForm()`, `updateAdminForm()`, `deleteAdminForm()`, `formsAnalytics()`, `formContactSegments()`, `formContactLists()`, `saveFormContactList()`, and `deleteFormContactList()` for authenticated custom admin clients. Existing `formSubmissions()`, `formSubmission()`, `updateFormSubmission()`, `formContacts()`, `updateFormContact()`, and `deleteFormContact()` also accept per-call admin auth options, site overrides, actor headers, and credentialed cookie requests so external form builders and CRM tools can use the Forms backend directly.
