...
- `POST /api/sites/:siteId/collections/:collectionId/records`
  ...
  - The SDK exposes authenticated CMS admin helpers for custom builders: `adminCollections()`, `createAdminCollection()`, `adminCollection()`, `updateAdminCollection()`, `deleteAdminCollection()`, `adminCollectionRecords()`, `createAdminCollectionRecord()`, `adminCollectionRecord()`, `updateAdminCollectionRecord()`, and `deleteAdminCollectionRecord()`. They use the same per-call admin auth options as live management and let external CMS UIs manage schemas and records without relying on Backy's admin route implementation.
