...
- `GET/PATCH/POST /api/admin/sites/:siteId/frontend-design`
  ...
  - `POST` with `{ action: "capture-content-template", resourceType: "page" | "blogPost" | "form" | "section", resourceId, templateId?, templateName?, routePattern? }` snapshots an existing page, blog post, form, or reusable section into a reusable frontend design template, preserves chrome/tokens/custom CSS/binding hints from frontend-design metadata, preserves structured form fields/settings and section canvas content, infers editable bindings from element props/data bindings, and emits `frontendDesign.template.capture` audit logs.
