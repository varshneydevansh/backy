...
- `PATCH /api/sites/:siteId/manage/blog/:postId`
  ...
  - Custom frontends using the SDK can call `listBackyContentElements()` to build a flat external-editor layer inventory with parent/depth/path metadata and editable target paths, then call `findBackyContentElement()`, `groupBackyContentElements()`, `ungroupBackyContentElements()`, `patchBackyContentElement()`, `patchBackyContentElements()`, `patchBackyContentEditableField()`, `patchBackyContentEditableFields()`, `patchBackyContentEditableMapEntry()`, `patchBackyContentEditableMapEntries()`, and `patchBackyContentEditableMapValues()` to apply Canva-style same-sibling grouping/ungrouping, one or many advertised editable target paths, render `editableMap`-style `{ elementId, field, value }` updates, keyed editable-map updates such as `hero.title -> props.content`, or form-state objects such as `{ "button.primary.color": "#111827" }` to nested page/blog content. Manifest `data.modules.liveManagement.editorComposition` and OpenAPI `x-backy-live-management.editorComposition` expose the `backy.editor-composition-commands.v1` command handoff with group/ungroup helper names, Cmd/Ctrl+G and Shift+Cmd/Ctrl+G shortcuts, same-parent/unlocked constraints, the `props.editorGroup` marker, and tablet/mobile responsive geometry preservation. The grouping helpers mark created groups with `props.editorGroup`, rewrite child `parentId`s, preserve desktop and tablet/mobile responsive geometry, and return changed content ready for conflict-safe live-management updates. `buildBackyLiveManagedPageEditableMapUpdate()` and `buildBackyLiveManagedBlogPostEditableMapUpdate()` package those patched content trees into full live-management update bodies with `expectedUpdatedAt`.
...
- `GET /api/sites/:siteId/media/:mediaId`
  ...
  - Custom media pickers should use manifest `data.modules.media.fileCategories` instead of hardcoded filters. The manifest maps `document` plus compatibility alias `file` to downloadable document assets, maps `font` to typography/font-manifest flows, and marks only `image` as transform/responsive eligible. Private delivery uses the manifest `deliveryPolicy` signed URL endpoint and never exposes private file tokens in public discovery responses.
  - Authenticated custom admin/editor shells should use manifest `data.modules.media.managementPolicy` (`backy.media-management.v1`) or OpenAPI `x-backy-media-file-categories.managementPolicy` to discover the admin media list/upload/update/delete, folder, retained-version, signed-url, binding, transform-preparation, and provider-analytics endpoints. The policy advertises accepted upload fields, SDK helper names, required media permissions, response contract markers, and non-secret credential handling without exposing private file tokens.
...
- `GET /api/admin/sites/:siteId/media`
  ...
  - Manifest/OpenAPI media management discovery mirrors these routes through `backy.media-management.v1`, including `uploadMedia`, `replaceMedia`, `createMediaSignedUrl`, `bindMedia`, `prepareMediaTransforms`, and `ingestMediaProviderAnalytics` helper names so custom Backy-compatible admin UIs can bind controls to the central media backend from the discovery document.
...
- CORS policy for custom frontends:
  ...
  - Backy completion status is discoverable without secrets through public manifest `data.contract.completionStatus` and site-scoped OpenAPI `x-backy-completion-status`. The contract uses `backy.completion-status.v1` to publish the current audited counts (`39 Ready / 6 Partial / 0 Prototype / 0 Missing`), the remaining Partial surfaces, each proving gate (`ci:forms-postgres`, `ci:sdk-postgres-smoke`, `ci:settings-provider-certification`, `ci:commerce-provider-certification`), workflow paths, required env aliases, runtime alias-presence booleans, and the non-secret privacy boundary so custom admin clients can explain exactly why Backy is not yet fully certified without scraping the page audit or receiving database/provider secrets.
