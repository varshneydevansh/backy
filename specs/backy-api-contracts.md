...
- `GET/PATCH/POST /api/admin/sites/:siteId/frontend-design`
  ...
  - `POST` with `{ action: "capture-content-template", resourceType: "page" | "blogPost", resourceId, templateId?, templateName?, routePattern? }` snapshots an existing page or blog canvas/content document into a reusable frontend design template, preserves chrome/tokens/custom CSS/binding hints from `meta.frontendDesign*`, infers editable bindings from element props/data bindings, and emits `frontendDesign.template.capture` audit logs.
