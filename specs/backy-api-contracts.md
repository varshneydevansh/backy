...
- `PATCH /api/sites/:siteId/manage/pages/:pageId`
  ...
  - Selected rendered elements expose quick layout controls for `x`, `y`, `width`, `height`, layer/z-index, rotation, visibility, and lock state; these patch the same top-level page element fields consumed by the public renderer/admin canvas, and locked elements block inline content/destination/media/appearance edits until unlocked.
