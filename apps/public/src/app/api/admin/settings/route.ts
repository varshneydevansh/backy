...
const getFrontendDatabaseCertificationRuntime = (
  ...
  const databaseUrlAlias = FRONTEND_DATABASE_CERTIFICATION_ENV_ALIASES.find((key) => Boolean(process.env[key]?.trim())) || null;
  ...
  return {
    ...
    readyForCertification: dataMode !== 'demo' && databaseUrlConfigured && disposableConfirmed,
    missing,
    secretHandling:
      'Database URLs and service credentials stay in CI/runtime environment variables; Settings admin API returns only alias names, booleans, and target guard presence.',
  };
};
...
const frontendDatabaseCertificationContract = (
  runtime: ReturnType<typeof getFrontendDatabaseCertificationRuntime>,
  scenarioEvidence: ReturnType<typeof buildFrontendDatabaseCertificationScenarioEvidence>,
) => ({
  generatedAt: new Date().toISOString(),
...
const toAdminSettings = (
  ...
  const frontendDatabaseCertificationRuntime = getFrontendDatabaseCertificationRuntime(runtimeDatabase, runtimePublicApi);
