...
const normalizeFulfillmentStatus = (value: string): FulfillmentDispatchPayload['status'] => {
  if (value === 'succeeded' || value === 'failed' || value === 'requires_action' || value === 'requested') return value;
...
const existingFulfillmentPayload = (record: CollectionRecordAuditSource): FulfillmentDispatchPayload | null => {
  const values = toRecord(record.values);
  ...
  return {
    ...
    provider: textValue(values.fulfillmentprovider) || textValue(values.fulfillmentcarrier) || 'manual',
    orderNumber: textValue(values.ordernumber) || record.slug,
    requestedAt: textValue(values.fulfillmentrequestedat) || record.updatedAt || new Date().toISOString(),
...
const buildFulfillmentDispatch = async (
  ...
  const orderFulfillmentStatus = textValue(values.fulfillmentstatus) || 'unfulfilled';
  if (orderFulfillmentStatus === 'fulfilled' || orderFulfillmentStatus === 'cancelled') {
    return {
      error: {
        status: 409,
  ...
  const provider = textValue(body.provider) || textValue(values.fulfillmentprovider) || textValue(values.fulfillmentcarrier) || 'manual';
  const status = normalizeFulfillmentStatus(textValue(body.status) || 'requires_action');
  const existingId = textValue(values.fulfillmentid);
  const id = existingId || `ful_${record.id}_${Date.now().toString(36)}`;
  const completedAt = status === 'succeeded' ? now : textValue(values.fulfillmentcompletedat) || null;
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (!shouldUseDemoStoreFallback()) {
      ...
      return NextResponse.json({ success: true, requestId, data: { record, fulfillment: existingFulfillmentPayload(record) } });
    }
  ...
  try {
    ...
    if (!shouldUseDemoStoreFallback()) {
      ...
      if (validationErrors.length > 0) {
        return errorResponse(400, 'VALIDATION_ERROR', 'Fulfillment values are invalid', requestId, validationErrors);
      }
      ...
      await recordAdminAudit({
        ...
        metadata: fulfillmentAuditMetadata(collection, updated, result.fulfillment),
        requestId,
      });

      return NextResponse.json({ success: true, requestId, data: { record: updated, order: updated, fulfillment: result.fulfillment, cacheInvalidation } });
    }
    ...
    return NextResponse.json({ success: true, requestId, data: { record: updated, order: updated, fulfillment: result.fulfillment } });
  } catch (error) {
