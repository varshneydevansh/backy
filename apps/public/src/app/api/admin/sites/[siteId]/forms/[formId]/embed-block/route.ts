...
import type { BackyJsonObject, BackyJsonValue, FormDefinition, FormFieldDefinition, SiteSettings } from '@backy-cms/core';
import { requireAdminAccess } from '@/lib/adminAccess';
import { recordAdminAudit } from '@/lib/adminAudit';
import { createReusableSection, getFormById, getReusableSectionByIdOrSlug, getSiteByIdOrSlug } from '@/lib/backyStore';
...
const uniqueSlug = async (
  ...
  const normalizedBase = normalizeSlug(baseSlug) || `form-embed-${Date.now().toString(36)}`;
  for (let index = 0; index < 50; index += 1) {
    const slug = index === 0 ? normalizedBase : `${normalizedBase}-${index + 1}`;
...
const fieldProps = (field: FormFieldDefinition): JsonRecord => {
  ...
  if (field.validation?.length) props.validation = field.validation as unknown as BackyJsonValue;
  return props;
};
...
const siteFrontendDesign = (settings: SiteSettings | undefined): NonNullable<SiteSettings['frontendDesign']> | undefined => (
  settings?.frontendDesign && settings.frontendDesign.status !== 'unconfigured'
    ? settings.frontendDesign
    : undefined
);

const settingRecord = (form: FormDefinition, key: string): Record<string, unknown> | undefined => {
...
const buildEmbedMetadata = (
  ...
  const templateId = settingText(form, 'frontendDesignTemplateId') || `form-embed-${form.id}`;
  const templateName = settingText(form, 'frontendDesignTemplateName') || `${formTitle(form)} embed block`;
  const frontendDesignTokens = settingRecord(form, 'frontendDesignTokens') || frontendDesign?.tokens;
  const frontendDesignChrome = settingRecord(form, 'frontendDesignChrome') || frontendDesign?.chrome;
  const frontendDesignSource = settingRecord(form, 'frontendDesignSource') || frontendDesign?.source;
  const frontendDesignCustomCss = settingText(form, 'frontendDesignCustomCss') || frontendDesign?.tokens?.customCss || '';
  const bindingHints = Array.isArray(recordValue(form.settings)?.frontendDesignBindingHints)
  ...
  return {
    ...
    frontendDesignSource: frontendDesignSource ? cloneJson(frontendDesignSource) as BackyJsonValue : null,
    frontendDesignTokens: frontendDesignTokens ? cloneJson(frontendDesignTokens) as BackyJsonValue : null,
    frontendDesignChrome: frontendDesignChrome ? cloneJson(frontendDesignChrome) as BackyJsonValue : null,
...
const buildFormEmbedContent = (
  ...
  const formElement: EmbedCanvasElement = {
    ...
    props: {
      ...
      method: 'POST',
      successMessage: form.successMessage || 'Thanks. We received your submission.',
      successRedirectUrl: form.successRedirectUrl || '',
      audience: form.audience,
      moderationMode: form.moderationMode || 'manual',
      frontendDesignTemplateId: settingText(form, 'frontendDesignTemplateId') || '',
      contactShare: form.contactShare ? cloneJson(form.contactShare) as BackyJsonValue : null,
      collectionTarget: form.collectionTarget ? cloneJson(form.collectionTarget) as BackyJsonValue : null,
    },
    styles: {
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const publicBaseUrl = textValue(body.publicBaseUrl) || new URL(request.url).origin;
    ...
    if (!shouldUseDemoStoreFallback()) {
      ...
      const sectionSlug = await uniqueSlug(site.id, textValue(body.slug) || `${formTitle(form)} form embed`, (slug) => repositories.reusableSections.getBySlug(site.id, slug).then(Boolean));
      ...
      const rawMetadata = buildEmbedMetadata(form, site.settings, { definitionUrl, submitUrl, sectionSlug });
      const metadata = buildInitialReusableSectionMetadata(rawMetadata, { actor, requestId }) as BackyJsonObject;
      const section = (await repositories.reusableSections.create({
        siteId: site.id,
        name: textValue(body.name) || `${formTitle(form)} embed block`,
        slug: sectionSlug,
        description: textValue(body.description) || `Reusable frontend embed block for ${formTitle(form)}.`,
        category: textValue(body.category) || 'forms',
        status: 'active',
        tags: ['form', 'embed', form.name].filter(Boolean),
        content: buildFormEmbedContent(form, site.settings, { definitionUrl, submitUrl }),
        metadata,
      ...
      return NextResponse.json(
        { success: true, requestId, data: { section, cacheInvalidation, embed: { definitionUrl, submitUrl } } },
        { status: 201 },
      );
    }
    ...
    const sectionSlug = await uniqueSlug(site.id, textValue(body.slug) || `${formTitle(form)} form embed`, (slug) => Boolean(getReusableSectionByIdOrSlug(site.id, slug)));
    const definitionUrl = `${publicBaseUrl}/api/sites/${encodeURIComponent(site.id)}/forms/${encodeURIComponent(form.id)}/definition`;
    const submitUrl = `${publicBaseUrl}/api/sites/${encodeURIComponent(site.id)}/forms/${encodeURIComponent(form.id)}/submissions`;
    const rawMetadata = buildEmbedMetadata(form, site.settings, { definitionUrl, submitUrl, sectionSlug });
    const section = createReusableSection(site.id, {
      ...
      tags: ['form', 'embed', form.name].filter(Boolean),
      content: buildFormEmbedContent(form, site.settings, { definitionUrl, submitUrl }),
      metadata: buildInitialReusableSectionMetadata(rawMetadata, { actor, requestId }),
      sourceElementId: form.id,
      createdBy: actor,
      updatedBy: actor,
    });
    ...
    return NextResponse.json(
      { success: true, requestId, data: { section, embed: { definitionUrl, submitUrl } } },
      { status: 201 },
    );
  } catch (error) {
