...
const deliverReusableSectionWebhook = async (params: {
  ...
  > | null;
  site: Site;
  action: "reusableSection.updated" | "reusableSection.deleted";
  before?: Parameters<typeof reusableSectionWebhookSnapshot>[0];
  ...
  deliverSiteWebhooks({
    ...
    data: {
      ...
      ...(params.before
        ? { before: reusableSectionWebhookSnapshot(params.before) }
        : {}),
      ...(params.after
    ...
    metadata: {
      ...
      category: (params.after || params.before)?.category || null,
  ...
  try {
    ...
    if (!shouldUseDemoStoreFallback()) {
      ...
      if (nextSlug && nextSlug !== section.slug) {
        const conflict = await repositories.reusableSections.getBySlug(
          site.id,
          nextSlug,
        );
      ...
      const metadata = buildReusableSectionUpdateMetadata(
        section,
        body.metadata === undefined ? undefined : parseMetadata(body.metadata),
        {
          actor: updatedBy,
          requestId,
        },
      ...
      const updated = (
        await repositories.reusableSections.update(site.id, section.id, {
          ...
          ...(body.content !== undefined
            ...
            : {}),
          metadata,
          ...(body.sourceElementId !== undefined
            ? {
                sourceElementId:
      ...
      await recordAdminAudit({
        ...
        after: updated,
        metadata: {
          changedKeys: Object.keys(body).filter(
            (key) => key !== "expectedVersion" && key !== "expectedUpdatedAt",
          ),
      ...
      await deliverReusableSectionWebhook({
        repositories,
        site,
        action: "reusableSection.updated",
        before: section,
        after: updated,
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (!shouldUseDemoStoreFallback()) {
      ...
      const deleted = await repositories.reusableSections.delete(
        site.id,
        section.id,
      );
      if (!deleted) {
      ...
      await deliverReusableSectionWebhook({
        repositories,
        site,
        action: "reusableSection.deleted",
        before: section,
        requestId,
        actor: access.session?.user.id,
      });
