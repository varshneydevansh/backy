...
const assertSettingsSourceContracts = () => {
  ...
  const requiredSettingsRouteSnippets = [
    ...
    'Frontend database scenario evidence',
...
const updateSettingsThroughUi = async (client, suffix, originalSettings, notificationWebhookUrl, siteScopeSite) => {
  ...
  const infrastructureState = await evaluate(client, `(() => {
    ...
    hasPaddleSubscriptionEnv: document.body?.innerText?.includes('Paddle API key') || document.body?.innerText?.includes('Paddle subscription API key') || false,
    ...
    hasReleaseCertificationCommerceHttpAliases: runbookText.includes('COMMERCE_TAX_PROVIDER_URL') && runbookText.includes('COMMERCE_SHIPPING_PROVIDER_URL') && runbookText.includes('COMMERCE_DISCOUNT_PROVIDER_URL') && runbookText.includes('COMMERCE_PRODUCT_SYNC_URL') && runbookText.includes('COMMERCE_SUBSCRIPTION_ACTION_URL'),
  };
})()`);
  ...
  const deliveryApiState = await evaluate(client, `(() => {
    ...
    return {
      ...
      hasFrontendDatabaseScenarioEvidence: text.includes('Frontend database scenario evidence') &&
        text.includes('backy.frontend-database-certification-evidence.v1') &&
        text.includes('Manifest and OpenAPI discovery') &&
        text.includes('Commerce contracts') &&
        text.includes('Generated SDK and cache'),
    };
  })()`);
  assert(
    deliveryApiState.hasSiteSettingsRead &&
      ...
      deliveryApiState.hasFrontendDatabaseScenarioEvidence,
