...
const assertCanvasEditorShortcutSource = () => {
  ...
  assert(source.includes('const selectedEntriesShareParent = selectedEntries.length > 0') && source.includes('const canSelectParentLayer = selectedEntriesShareParent && Boolean(selectedParentId)') && source.includes('data-testid="editor-inspector-select-parent-layer"') && source.includes('Select parent layer (Shift+Enter)'), 'Editor multi-selection inspector must allow selected child siblings to jump back to their common parent layer');
  ...
  assert(source.includes('data-testid="editor-send-to-back"') && source.includes('aria-keyshortcuts="Shift+Control+[ Shift+Meta+["') && source.includes('aria-keyshortcuts="Control+[ Meta+["') && source.includes('aria-keyshortcuts="Control+] Meta+]"') && source.includes('aria-keyshortcuts="Shift+Control+] Shift+Meta+]"'), 'Editor toolbar layer-order controls must expose bracket shortcut metadata');
  ...
  assert(source.includes('data-testid="editor-distribute-horizontal"') && source.includes('data-testid="editor-distribute-vertical"') && source.includes('data-testid="editor-inspector-distribute-horizontal"') && source.includes('Distribute selected layers horizontally') && source.includes('Distribute selected layers vertically'), 'Editor toolbar and inspector must expose labelled distribution controls for selected layers');
...
const assertComponentLibraryEmptyStateSource = () => {
  ...
  assert(propertyPanelSource.includes("schema: 'backy.editor.repeater-dataset-action-plan.v1'") && propertyPanelSource.includes('data-testid="editor-repeater-dataset-action-plan"') && propertyPanelSource.includes('actionPlan: repeaterDatasetActionPlan'), 'Editor Data panel must render and export repeater dataset action plans');
