...
assert(routeSource.includes("const NEWSLETTER_AUDIENCE_FILTERS") && routeSource.includes("id: 'send-ready'") && routeSource.includes("id: 'held'") && routeSource.includes("id: 'unsubscribed'"), 'Newsletter subscriber management must expose send-ready, held, and unsubscribed audience views.');
assert(routeSource.includes('buildAudienceFilterCounts(subscribers)') && routeSource.includes('subscriberMatchesAudienceFilter(contact,') && routeSource.includes('isSubscriberSendReady(contact)'), 'Newsletter audience filters must derive counts from canonical subscriber readiness logic.');
assert(routeSource.includes('data-testid="newsletter-subscriber-controls"') && routeSource.includes('data-audience-filter={subscriberAudienceFilter}') && routeSource.includes('data-visible-subscriber-count={latestSubscribers.length}'), 'Newsletter subscriber controls must expose machine-readable current audience and visible count metadata.');
assert(routeSource.includes('data-testid={`newsletter-audience-filter-${filter.id}`}') && routeSource.includes('aria-pressed={active}') && routeSource.includes('data-filter-count={count}'), 'Newsletter audience filter buttons must be accessible and expose per-filter counts.');
assert(routeSource.includes('data-testid="newsletter-subscriber-search"') && routeSource.includes('buildSubscriberSearchText(subscriber)'), 'Newsletter subscriber management must include searchable subscriber, topic, source, and form metadata.');
assert(routeSource.includes('data-testid="newsletter-subscriber-visible-count"') && routeSource.includes('Export uses the visible audience view.'), 'Newsletter subscriber list must tell operators how many records are visible and what export will include.');
assert(routeSource.includes('Exported ${latestSubscribers.length} visible newsletter subscriber') && routeSource.includes('No subscribers match the current audience view.'), 'Newsletter CSV export must follow the visible audience view and explain empty filtered exports.');
