...
| `/products` and `/orders` | `npm run ci:commerce-provider-certification` | `npm run test:partial-gate-preflights` | `npm run test:admin-contract-source` | Commerce provider workflow or `certify_commerce_providers` with selected payment, tax, shipping, discount, catalog, subscription, and webhook provider inputs |
