...
Use `npm run test:custom-frontend-connection` when connecting a separate website frontend. With `BACKY_CUSTOM_FRONTEND_API_BASE_URL=https://<backy-public-domain>/api` and `BACKY_CUSTOM_FRONTEND_SITE_ID=<site-id-or-slug>`, it proves the public site discovery, agent-handoff, manifest, OpenAPI, resolve, and render contracts. Add `BACKY_CUSTOM_FRONTEND_URL=https://<frontend-domain>` and `BACKY_CUSTOM_FRONTEND_REQUIRE_FRONTEND=1` after deploying the website to prove the rendered DOM still exposes Backy element ids, element types, component-contract pointers, and editable-map pointers.
