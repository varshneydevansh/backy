...
assert(customFrontendAgentHandoff.contentCreation?.blogChildStarterTemplates?.some((template) => template.id === 'investigation-report' && template.intent === 'investigation' && template.backyCanvasRoute?.includes('starterTemplate=investigation-report')), 'manifest() custom frontend agent handoff missing investigation report blog child starter route');
assert(customFrontendAgentHandoff.contentCreation?.blogChildStarterTemplates?.some((template) => template.id === 'audio-transcript' && template.intent === 'audio-transcript' && template.sections?.includes('Audio player') && template.sections?.includes('Transcript') && template.sections?.includes('Files')), 'manifest() custom frontend agent handoff missing audio transcript blog child starter route');
