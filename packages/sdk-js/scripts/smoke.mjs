...
assert(customFrontendAgentHandoff.contentCreation?.blogChildStarterTemplates?.length >= 33, 'manifest() custom frontend agent handoff missing full blog child starter catalog');
assert(customFrontendAgentHandoff.contentCreation?.blogChildStarterTemplates?.some((template) => template.id === 'survey' && template.intent === 'investigation' && template.backyCanvasRoute?.includes('starterTemplate=survey')), 'manifest() custom frontend agent handoff missing investigation blog child starter route');
assert(customFrontendAgentHandoff.contentCreation?.blogChildStarterTemplates?.some((template) => template.id === 'gallery' && template.intent === 'audio-transcript' && template.sections?.includes('Audio player') && template.sections?.includes('Transcript') && template.sections?.includes('Files')), 'manifest() custom frontend agent handoff missing audio transcript blog child starter route');
assert(customFrontendAgentHandoff.contentCreation?.blogChildStarterTemplates?.some((template) => template.id === 'newsletter' && template.intent === 'newsletter' && template.customFrontendRoute?.includes('frontendDesignTemplateId=:templateId')), 'manifest() custom frontend agent handoff missing newsletter custom frontend blog child starter route');
assert(customFrontendAgentHandoff.contentCreation?.blogTemplateInheritance?.schemaVersion === 'backy.blog-child-template-inheritance.v1', 'manifest() custom frontend agent handoff missing blog template inheritance schema');
...
assert(agentHandoff.data.contentCreation?.blogChildStarterTemplates?.length >= 33, 'customFrontendAgentHandoff() missing full blog child starter catalog');
assert(agentHandoff.data.contentCreation?.blogChildStarterTemplates?.some((template) => template.id === 'gallery' && template.intent === 'audio-transcript' && template.sections?.includes('Audio player') && template.sections?.includes('Transcript') && template.sections?.includes('Files')), 'customFrontendAgentHandoff() missing audio transcript media/file blog child starter');
assert(agentHandoff.data.apiAlignment?.blogChildStarterTemplates?.length === agentHandoff.data.contentCreation?.blogChildStarterTemplates?.length, 'customFrontendAgentHandoff() API alignment blog child catalog drifted');
