...
The SDK smoke now executes the non-destructive Settings action helpers locally: infrastructure diagnostics, storage provisioning readiness, credential-rotation readiness, Vercel secret-manager dry-run planning, and notification webhook delivery through an ephemeral local receiver. Live Supabase/Vercel/storage/notification provider certification remains gated by the Settings provider workflow and real credentials.
