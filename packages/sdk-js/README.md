...
Authenticated custom admin clients can also call `commerceOrderAnalytics()` to read private/no-store order analytics plus the Orders provider-certification evidence, `commerceProductProviderSync(productId)` to inspect a product's current catalog-sync handoff, and `syncCommerceProductProvider(productId, { provider })` to execute the audited provider-sync route when the admin has `commerce.edit`. These methods return bounded non-secret handoff metadata and require the same per-call admin auth options as the other admin SDK methods.
