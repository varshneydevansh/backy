...
export const CUSTOM_FRONTEND_BLOG_CHILD_STARTER_TEMPLATES = [
  ...
  { id: 'investigation-report', name: 'Investigation report', intent: 'investigation', sections: ['Report hero', 'Evidence timeline', 'Sources'] },
  { id: 'audio-transcript', name: 'Audio transcript', intent: 'audio-transcript', sections: ['Audio player', 'Transcript', 'Files'] },
  { id: 'case-study', name: 'Case study', intent: 'case-study', sections: ['Problem', 'Process', 'Outcome'] },
...
export const CUSTOM_FRONTEND_COMPONENT_TYPE_CONTRACTS = [
  ...
  componentTypeContract('audio', 'media', 'Audio', ['props.src', 'props.mediaId', 'props.caption', 'props.transcript', 'props.controls', 'props.autoplay', 'props.loop'], { supportsMediaAssets: true }),
...
export const buildCustomFrontendDeploymentTopology = (siteId: string) => ({
  ...
  projects: {
    ...
    customFrontend: {
      ...
      browserSafeEnv: ['NEXT_PUBLIC_BACKY_API_BASE_URL', 'NEXT_PUBLIC_BACKY_SITE_ID', 'NEXT_PUBLIC_BACKY_SITE_PUBLIC_HOST'],
      serverSideEnv: ['BACKY_PUBLIC_API_BASE_URL', 'BACKY_SITE_ID', 'BACKY_SITE_PUBLIC_HOST'],
      domainAttachment: 'Attach the public website host to the custom frontend Vercel project, not to backy-admin or backy-public.',
      apiOriginPolicy: 'Use backy-public as the API/render origin unless a dedicated API domain is configured.',
      forbiddenEnv: [
        'Backy admin URLs',
...
export const buildCustomFrontendAgentHandoff = (
  ...
  rules: [
    ...
    'Deploy protected backy-admin, public backy-public, and separate custom frontend projects with the browser-safe and server-side env boundary advertised in deploymentTopology.',
  ],
  privacy: {
