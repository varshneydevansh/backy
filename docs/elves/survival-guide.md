...
**Single next action:** Create and publish the real Backy site record for `devanshvarshney`/`devanshvarshney.com` in admin first. Once `GET https://backy-public.vercel.app/api/sites?identifier=devanshvarshney` and the home render are ready, create the separate custom website frontend either from Site Detail -> Separate custom frontend project -> Download starter project plus `npm run custom-frontend:materialize -- --manifest <downloaded-starter-json> --out <target-dir>`, or directly with `npm run custom-frontend:scaffold -- --site-id devanshvarshney --public-host devanshvarshney.com --api-base https://backy-public.vercel.app/api --out <target-dir>`. Attach the public website domain to that frontend project, then use Site Detail -> Separate custom frontend project -> Verify deployed frontend and rerun `npm run test:custom-frontend-connection` with `BACKY_CUSTOM_FRONTEND_URL=<frontend-domain>`, `BACKY_CUSTOM_FRONTEND_REQUIRE_FRONTEND=1`, and `BACKY_CUSTOM_FRONTEND_REQUIRE_PROBE=1` before broad UI polish.
