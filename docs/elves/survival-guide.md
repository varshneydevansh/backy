...
**Previous deploy verification:** Backy source through `91b05c86` is pushed. The latest `backy-public` and `backy-admin` production deployments were Ready before this template-registry source slice, the real-site live custom frontend connection gate now passes 69 checks against `https://backy-public.vercel.app/api` plus the deployed custom frontend, and production readiness passes 47 checks. This template-registry slice still needs its own commit/push and, if Vercel auto-deploys from `main`, a fresh deployment log check.
